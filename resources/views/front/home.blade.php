@extends('front.layouts.app_home')
@section('main_section')
@include('front.partials.main')
@endsection
@section('content')

<div class="centerside">
            <div class="centersidecont">
              <div class="row">
                 @include('front.partials.home.slider')
  
                
                                <div class="col-lg-12" style="display: none">
                                    <div class="ServicesNew">
                                        <div class="owl-carousel owl-theme servItemOwlC">
                                      
                                            <div class="item">
                                                <a href="#" class="seervItem">
                                                    <div class="icon">
                                                        <img src="{{asset('frontUI/assets/img/cladreximg/Product/cat1.png')}}">
                                                    </div>
                                                    <div class="ServName">
                                                        <p>Laptop</p>
                                                    </div>
                                                </a>
                                            </div>
                                            
                                            <div class="item">
                                                <a href="#" class="seervItem">
                                                    <div class="icon">
                                                        <img src="{{asset('frontUI/assets/img/cladreximg/Product/cat2.png')}}">
                                                    </div>
                                                    <div class="ServName">
                                                        <p>Game Consoles</p>
                                                    </div>
                                                </a>
                                            </div>
                                            
                                            <div class="item">
                                                <a href="#" class="seervItem">
                                                    <div class="icon">
                                                        <img src="{{asset('frontUI/assets/img/cladreximg/Product/cat3.png')}}">
                                                    </div>
                                                    <div class="ServName">
                                                        <p>Audio & Video</p>
                                                    </div>
                                                </a>
                                            </div>
                                            
                                            <div class="item">
                                                <a href="#" class="seervItem">
                                                    <div class="icon">
                                                        <img src="{{asset('frontUI/assets/img/cladreximg/Product/cat4.png')}}">
                                                    </div>
                                                    <div class="ServName">
                                                        <p>Bottoms</p>
                                                    </div>
                                                </a>
                                            </div>
                                            
                                            <div class="item">
                                                <a href="#" class="seervItem">
                                                    <div class="icon">
                                                        <img src="{{asset('frontUI/assets/img/cladreximg/Product/cat5.png')}}">
                                                    </div>
                                                    <div class="ServName">
                                                        <p>Accessories</p>
                                                    </div>
                                                </a>
                                            </div>
                                            
                                            <div class="item">
                                                <a href="#" class="seervItem">
                                                    <div class="icon">
                                                        <img src="{{asset('frontUI/assets/img/cladreximg/Product/cat6.png')}}">
                                                    </div>
                                                    <div class="ServName">
                                                        <p>Smartphone</p>
                                                    </div>
                                                </a>
                                            </div>
                                            
                                            <div class="item">
                                                <a href="#" class="seervItem">
                                                    <div class="icon">
                                                        <img src="{{asset('frontUI/assets/img/cladreximg/Product/cat7.png')}}">
                                                    </div>
                                                    <div class="ServName">
                                                        <p>Tops & Sets</p>
                                                    </div>
                                                </a>
                                            </div>
                                            
                                            <div class="item">
                                                <a href="#" class="seervItem">
                                                    <div class="icon">
                                                        <img src="{{asset('frontUI/assets/img/cladreximg/Product/cat8.png')}}">
                                                    </div>
                                                    <div class="ServName">
                                                        <p>Video Games</p>
                                                    </div>
                                                </a>
                                            </div>
                                            
                                            <div class="item">
                                                <a href="#" class="seervItem">
                                                    <div class="icon">
                                                        <img src="{{asset('frontUI/assets/img/cladreximg/Product/cat9.png')}}">
                                                    </div>
                                                    <div class="ServName">
                                                        <p> Games & Consoles </p>
                                                    </div>
                                                </a>
                                            </div>
                                            
                                            <div class="item">
                                                <a href="#" class="seervItem">
                                                    <div class="icon">
                                                        <img src="{{asset('frontUI/assets/img/cladreximg/Product/cat10.png')}}">
                                                    </div>
                                                    <div class="ServName">
                                                        <p>Home</p>
                                                    </div>
                                                </a>
                                            </div>
                                          
                                            
                                            
                                        </div>
                                    </div>
                                </div>
                               
                               <div class="col-lg-12" style="display: none">
                                    <div class="NewPreview">
                                        <div class="row">
                                            <div class="col-md-12">
                                                <div class="banner-image banner-2">
                                                    <a href="#">
                                                        <img src="{{asset('frontUI/assets/img/cladreximg/Product/preview1.png')}}" alt="banner">
                                                        
                                                    </a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                             
                
                
                                <div class="col-lg-12" style="display: none">
                                    <div class="VerticalProduct">
                                        <div class="row">
                                            <div class="col-lg-6 col-md-12 col-sm-12 col-xs-12">
                                                <div class="ProVertItem">
                                                    <div class="row">
                                                        <div class="col-lg-5 col-md-5 col-sm-5 col-xs-12">
                                                            <div class="sssas">
                                                                
                                                                <div class="images">
                                                                
                                                                <img src="{{asset('frontUI/assets/img/cladreximg/Product/26.png')}}">
                                                            </div>
                                                            <div class="buttonsssRight">
                                                                    <ul>
                                                                        <li>
                                                                            <a href="#">
                                                                                <i class="bx bx-heart"></i>
                                                                            </a>
                                                                        </li>
                                                                        <li>
                                                                            <a href="#">
                                                                                <i class="bx bx-sync"></i>
                                                                            </a>
                                                                        </li>
                                                                        
                                                                    </ul>
                                                                </div>
                                                            </div>
                                                            
                                                        </div>
                                                        <div class="col-lg-7 col-md-7 col-sm-7 col-xs-12">
                                                            <div class="Dispflex">
                                                                
                                                                
                                                                <div class="Detailss">
                                                                    <h2>Game Consoles</h2>
                                                                    <a href="#">New Balance Arishi Sport v1Couture Juicy eu</a>
                                                                    <div class="product_price">
                                                                        <span class="price_sale">US $300-450 </span>
                                                                    </div>
                                                                    <div class="min_order" title="Min. Order: 1 Piece">
                                                                        <span class="label">Min. Order: </span>
                                                                        <span class="value">1 Piece</span>
                                                                    </div>
                                                                    <div class="finshlinks">
                                                                        <ul>
                                                                            <li>
                                                                                <p class="text-muted mt-3">
                                                                                    <i class="bx bxs-star text-warning"></i>
                                                                                    <i class="bx bxs-star text-warning"></i>
                                                                                    <i class="bx bxs-star text-warning"></i>
                                                                                    <i class="bx bxs-star text-warning"></i>
                                                                                    <i class="bx bxs-star"></i>
                                                                                </p>
                                                                            </li>
                                                                            <li class="addbuton">
                                                                                <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                                                                    <i class="bx bx-basket" aria-hidden="true"></i>
                                                                                </button>
                                                                            </li>
                                                                        </ul>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            
                                                        </div>
                                                    </div>
                                                    <div class="saleee">
                                                        <p>-50%</p>
                                                    </div>
                                                </div>
                                            </div>
                                            
                                             <div class="col-lg-6 col-md-12 col-sm-12 col-xs-12">
                                                <div class="ProVertItem">
                                                    <div class="row">
                                                        <div class="col-lg-5 col-md-5 col-sm-5 col-xs-12">
                                                            <div class="sssas">
                                                                
                                                                <div class="images">
                                                                
                                                                <img src="{{asset('frontUI/assets/img/cladreximg/Product/27.png')}}">
                                                            </div>
                                                            <div class="buttonsssRight">
                                                                    <ul>
                                                                        <li>
                                                                            <a href="#">
                                                                                <i class="bx bx-heart"></i>
                                                                            </a>
                                                                        </li>
                                                                        <li>
                                                                            <a href="#">
                                                                                <i class="bx bx-sync"></i>
                                                                            </a>
                                                                        </li>
                                                                        
                                                                    </ul>
                                                                </div>
                                                            </div>
                                                            
                                                        </div>
                                                        <div class="col-lg-7 col-md-7 col-sm-7 col-xs-12">
                                                            <div class="Dispflex">
                                                                
                                                                
                                                                <div class="Detailss">
                                                                    <h2>Game Consoles</h2>
                                                                    <a href="#">New Balance Arishi Sport v1Couture Juicy eu</a>
                                                                    <div class="product_price">
                                                                        <span class="price_sale">US $300-450 </span>
                                                                    </div>
                                                                    <div class="min_order" title="Min. Order: 1 Piece">
                                                                        <span class="label">Min. Order: </span>
                                                                        <span class="value">1 Piece</span>
                                                                    </div>
                                                                    <div class="finshlinks">
                                                                        <ul>
                                                                            <li>
                                                                                <p class="text-muted mt-3">
                                                                                    <i class="bx bxs-star text-warning"></i>
                                                                                    <i class="bx bxs-star text-warning"></i>
                                                                                    <i class="bx bxs-star text-warning"></i>
                                                                                    <i class="bx bxs-star text-warning"></i>
                                                                                    <i class="bx bxs-star"></i>
                                                                                </p>
                                                                            </li>
                                                                            <li class="addbuton">
                                                                                <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                                                                    <i class="bx bx-basket" aria-hidden="true"></i>
                                                                                </button>
                                                                            </li>
                                                                        </ul>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            
                                                        </div>
                                                    </div>
                                                    <div class="saleee">
                                                        <p>-50%</p>
                                                    </div>
                                                </div>
                                            </div>
                                            
                                             <div class="col-lg-6 col-md-12 col-sm-12 col-xs-12">
                                                <div class="ProVertItem">
                                                    <div class="row">
                                                        <div class="col-lg-5 col-md-5 col-sm-5 col-xs-12">
                                                            <div class="sssas">
                                                                
                                                                <div class="images">
                                                                
                                                                <img src="{{asset('frontUI/assets/img/cladreximg/Product/28.png')}}">
                                                            </div>
                                                            <div class="buttonsssRight">
                                                                    <ul>
                                                                        <li>
                                                                            <a href="#">
                                                                                <i class="bx bx-heart"></i>
                                                                            </a>
                                                                        </li>
                                                                        <li>
                                                                            <a href="#">
                                                                                <i class="bx bx-sync"></i>
                                                                            </a>
                                                                        </li>
                                                                        
                                                                    </ul>
                                                                </div>
                                                            </div>
                                                            
                                                        </div>
                                                        <div class="col-lg-7 col-md-7 col-sm-7 col-xs-12">
                                                            <div class="Dispflex">
                                                                
                                                                
                                                                <div class="Detailss">
                                                                    <h2>Game Consoles</h2>
                                                                    <a href="#">New Balance Arishi Sport v1Couture Juicy eu</a>
                                                                    <div class="product_price">
                                                                        <span class="price_sale">US $300-450 </span>
                                                                    </div>
                                                                    <div class="min_order" title="Min. Order: 1 Piece">
                                                                        <span class="label">Min. Order: </span>
                                                                        <span class="value">1 Piece</span>
                                                                    </div>
                                                                    <div class="finshlinks">
                                                                        <ul>
                                                                            <li>
                                                                                <p class="text-muted mt-3">
                                                                                    <i class="bx bxs-star text-warning"></i>
                                                                                    <i class="bx bxs-star text-warning"></i>
                                                                                    <i class="bx bxs-star text-warning"></i>
                                                                                    <i class="bx bxs-star text-warning"></i>
                                                                                    <i class="bx bxs-star"></i>
                                                                                </p>
                                                                            </li>
                                                                            <li class="addbuton">
                                                                                <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                                                                    <i class="bx bx-basket" aria-hidden="true"></i>
                                                                                </button>
                                                                            </li>
                                                                        </ul>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            
                                                        </div>
                                                    </div>
                                                    <div class="saleee">
                                                        <p>-50%</p>
                                                    </div>
                                                </div>
                                            </div>
                                            
                                             <div class="col-lg-6 col-md-12 col-sm-12 col-xs-12">
                                                <div class="ProVertItem">
                                                    <div class="row">
                                                        <div class="col-lg-5 col-md-5 col-sm-5 col-xs-12">
                                                            <div class="sssas">
                                                                
                                                                <div class="images">
                                                                
                                                                <img src="{{asset('frontUI/assets/img/cladreximg/Product/29.png')}}">
                                                            </div>
                                                            <div class="buttonsssRight">
                                                                    <ul>
                                                                        <li>
                                                                            <a href="#">
                                                                                <i class="bx bx-heart"></i>
                                                                            </a>
                                                                        </li>
                                                                        <li>
                                                                            <a href="#">
                                                                                <i class="bx bx-sync"></i>
                                                                            </a>
                                                                        </li>
                                                                        
                                                                    </ul>
                                                                </div>
                                                            </div>
                                                            
                                                        </div>
                                                        <div class="col-lg-7 col-md-7 col-sm-7 col-xs-12">
                                                            <div class="Dispflex">
                                                                
                                                                
                                                                <div class="Detailss">
                                                                    <h2>Game Consoles</h2>
                                                                    <a href="#">New Balance Arishi Sport v1Couture Juicy eu</a>
                                                                    <div class="product_price">
                                                                        <span class="price_sale">US $300-450 </span>
                                                                    </div>
                                                                    <div class="min_order" title="Min. Order: 1 Piece">
                                                                        <span class="label">Min. Order: </span>
                                                                        <span class="value">1 Piece</span>
                                                                    </div>
                                                                    <div class="finshlinks">
                                                                        <ul>
                                                                            <li>
                                                                                <p class="text-muted mt-3">
                                                                                    <i class="bx bxs-star text-warning"></i>
                                                                                    <i class="bx bxs-star text-warning"></i>
                                                                                    <i class="bx bxs-star text-warning"></i>
                                                                                    <i class="bx bxs-star text-warning"></i>
                                                                                    <i class="bx bxs-star"></i>
                                                                                </p>
                                                                            </li>
                                                                            <li class="addbuton">
                                                                                <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                                                                    <i class="bx bx-basket" aria-hidden="true"></i>
                                                                                </button>
                                                                            </li>
                                                                        </ul>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            
                                                        </div>
                                                    </div>
                                                    <div class="saleee">
                                                        <p>-50%</p>
                                                    </div>
                                                </div>
                                            </div>
                                            
                                            
                                            
                                           
                                            
                                            
                                        </div>
                                    </div>
                                </div>
                             
                             <div class="col-lg-12" style="display: none">
                                    <div class="NewPreview">
                                        <div class="row">
                                            <div class="col-md-6">
                                                <div class="banner-image banner-2">
                                                    <a href="#">
                                                       
                                                        <img src="{{asset('frontUI/assets/img/cladreximg/Product/media1.png')}}">
                                                    </a>
                                                </div>
                                            </div>
                                            <div class="col-md-6">
                                                <div class="banner-image banner-2">
                                                    <a href="#">
                                                        <img src="{{asset('frontUI/assets/img/cladreximg/Product/media2.png')}}">
                                                    </a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                
                                
                                                                <div class="col-lg-12" style="display: none">
                                    <div class="productOwl PopuProd">
                                        <div class="title">
                                            <h2>Electronic <span>Product</span></h2>
                                        </div>
                                        <div class="Products PopuProdtemsss">
                                            <div class="owl-carousel owl-theme prprprOwl">
                                                <div class="item">
                                                <div class="PopuProdItem">
                                                        <div class="imges">
                                                            <img src="{{asset('frontUI/assets/img/cladreximg/Product/26.png')}}">
                                                        </div>
                                                        <div class="buttonsssRight">
                                                            <ul>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-heart"></i>
                                                                    </a>
                                                                </li>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-sync"></i>
                                                                    </a>
                                                                </li>
                                                                
                                                            </ul>
                                                        </div>
                                                        <div class="details">
                                                            <div class="categoru">
                                                                <a href="#" class="catlinl">zara official</a>
                                                            </div>
                                                            <div class="nameprod">
                                                                <a href="NewProductdetails.html" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                                                            </div>
                                                            <div class="price">
                                                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                                                    <!--<span class="label">FOB Price: </span>-->
                                                                    <span class="value">US $300-450 </span>
                                                                    <!--<span class="unit">/ Piece</span>-->
                                                                </div>
                                                            </div>
                                                            <div class="min_order" title="Min. Order: 1 Piece">
                                                                <span class="label">Min. Order: </span>
                                                                <span class="value">1 Piece</span>
                                                            </div>


                                                            <div class="finshlinks">
                                                                <ul>
                                                                    <li>
                                                                        <p class="text-muted mt-3">
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star"></i>
                                                                        </p>
                                                                    </li>
                                                                    
                                                                    <li class="addbuton">
                                                                        <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                                    <i class="bx bx-basket"></i>
                                                </button>
                                                                    </li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                    </div>
                                                      
                                                
                                                </div>
                                                <div class="item">
                                                <div class="PopuProdItem">
                                                        <div class="imges">
                                                            <img src="{{asset('frontUI/assets/img/cladreximg/Product/27.png')}}">
                                                        </div>
                                                        <div class="buttonsssRight">
                                                            <ul>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-heart"></i>
                                                                    </a>
                                                                </li>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-sync"></i>
                                                                    </a>
                                                                </li>
                                                                
                                                            </ul>
                                                        </div>
                                                        <div class="details">
                                                            <div class="categoru">
                                                                <a href="#" class="catlinl">zara official</a>
                                                            </div>
                                                            <div class="nameprod">
                                                                <a href="NewProductdetails.html" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                                                            </div>
                                                            <div class="price">
                                                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                                                    <span class="label">FOB Price: </span>
                                                                    <span class="value">US $300-450 </span>
                                                                    <span class="unit">/ Piece</span>
                                                                </div>
                                                            </div>
                                                            <div class="min_order" title="Min. Order: 1 Piece">
                                                                <span class="label">Min. Order: </span>
                                                                <span class="value">1 Piece</span>
                                                            </div>


                                                            <div class="finshlinks">
                                                                <ul>
                                                                    <li>
                                                                        <p class="text-muted mt-3">
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star "></i>
                                                                            <i class="bx bxs-star"></i>
                                                                        </p>
                                                                    </li>
                                                                    <li class="addbuton">
                                                                        <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                                    <i class="bx bx-basket"></i>
                                                </button>
                                                                    </li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                    </div>
                                                      
                                                
                                                </div>
                                                
                                                <div class="item">
                                                <div class="PopuProdItem">
                                                        <div class="imges">
                                                            <img src="{{asset('frontUI/assets/img/cladreximg/Product/28.png')}}">
                                                        </div>
                                                        <div class="buttonsssRight">
                                                            <ul>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-heart"></i>
                                                                    </a>
                                                                </li>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-sync"></i>
                                                                    </a>
                                                                </li>
                                                                
                                                            </ul>
                                                        </div>
                                                        <div class="details">
                                                            <div class="categoru">
                                                                <a href="#" class="catlinl">zara official</a>
                                                            </div>
                                                            <div class="nameprod">
                                                                <a href="NewProductdetails.html" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                                                            </div>
                                                            <div class="price">
                                                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                                                    <span class="label">FOB Price: </span>
                                                                    <span class="value">US $300-450 </span>
                                                                    <span class="unit">/ Piece</span>
                                                                </div>
                                                            </div>
                                                            <div class="min_order" title="Min. Order: 1 Piece">
                                                                <span class="label">Min. Order: </span>
                                                                <span class="value">1 Piece</span>
                                                            </div>


                                                            <div class="finshlinks">
                                                                <ul>
                                                                    <li>
                                                                        <p class="text-muted mt-3">
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star "></i>
                                                                            <i class="bx bxs-star "></i>
                                                                            <i class="bx bxs-star"></i>
                                                                        </p>
                                                                    </li>
                                                                    <li class="addbuton">
                                                                        <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                                    <i class="bx bx-basket"></i>
                                                </button>
                                                                    </li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                    </div>
                                                      
                                                
                                                </div>
                                                
                                                <div class="item">
                                                <div class="PopuProdItem">
                                                        <div class="imges">
                                                            <img src="{{asset('frontUI/assets/img/cladreximg/Product/29.png')}}">
                                                        </div>
                                                        <div class="buttonsssRight">
                                                            <ul>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-heart"></i>
                                                                    </a>
                                                                </li>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-sync"></i>
                                                                    </a>
                                                                </li>
                                                                
                                                            </ul>
                                                        </div>
                                                        <div class="details">
                                                            <div class="categoru">
                                                                <a href="#" class="catlinl">zara official</a>
                                                            </div>
                                                            <div class="nameprod">
                                                                <a href="NewProductdetails.html" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                                                            </div>
                                                            <div class="price">
                                                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                                                    <span class="label">FOB Price: </span>
                                                                    <span class="value">US $300-450 </span>
                                                                    <span class="unit">/ Piece</span>
                                                                </div>
                                                            </div>
                                                            <div class="min_order" title="Min. Order: 1 Piece">
                                                                <span class="label">Min. Order: </span>
                                                                <span class="value">1 Piece</span>
                                                            </div>


                                                            <div class="finshlinks">
                                                                <ul>
                                                                    <li>
                                                                        <p class="text-muted mt-3">
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star"></i>
                                                                            <i class="bx bxs-star "></i>
                                                                            <i class="bx bxs-star"></i>
                                                                        </p>
                                                                    </li>
                                                                    <li class="addbuton">
                                                                        <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                                    <i class="bx bx-basket"></i>
                                                </button>
                                                                    </li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                    </div>
                                                      
                                                
                                                </div>
                                                
                                                <div class="item">
                                                <div class="PopuProdItem">
                                                        <div class="imges">
                                                            <img src="{{asset('frontUI/assets/img/cladreximg/Product/26.png')}}">
                                                        </div>
                                                        <div class="buttonsssRight">
                                                            <ul>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-heart"></i>
                                                                    </a>
                                                                </li>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-sync"></i>
                                                                    </a>
                                                                </li>
                                                                
                                                            </ul>
                                                        </div>
                                                        <div class="details">
                                                            <div class="categoru">
                                                                <a href="#" class="catlinl">zara official</a>
                                                            </div>
                                                            <div class="nameprod">
                                                                <a href="NewProductdetails.html" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                                                            </div>
                                                            <div class="price">
                                                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                                                    <span class="label">FOB Price: </span>
                                                                    <span class="value">US $300-450 </span>
                                                                    <span class="unit">/ Piece</span>
                                                                </div>
                                                            </div>
                                                            <div class="min_order" title="Min. Order: 1 Piece">
                                                                <span class="label">Min. Order: </span>
                                                                <span class="value">1 Piece</span>
                                                            </div>


                                                            <div class="finshlinks">
                                                                <ul>
                                                                    <li>
                                                                        <p class="text-muted mt-3">
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star"></i>
                                                                            <i class="bx bxs-star "></i>
                                                                            <i class="bx bxs-star"></i>
                                                                        </p>
                                                                    </li>
                                                                    <li class="addbuton">
                                                                        <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                                    <i class="bx bx-basket"></i>
                                                </button>
                                                                    </li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                    </div>
                                                      
                                                
                                                </div>
                                                
                                                <div class="item">
                                                <div class="PopuProdItem">
                                                        <div class="imges">
                                                            <img src="{{asset('frontUI/assets/img/cladreximg/Product/27.png')}}">
                                                        </div>
                                                        <div class="buttonsssRight">
                                                            <ul>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-heart"></i>
                                                                    </a>
                                                                </li>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-sync"></i>
                                                                    </a>
                                                                </li>
                                                                
                                                            </ul>
                                                        </div>
                                                        <div class="details">
                                                            <div class="categoru">
                                                                <a href="#" class="catlinl">zara official</a>
                                                            </div>
                                                            <div class="nameprod">
                                                                <a href="NewProductdetails.html" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                                                            </div>
                                                            <div class="price">
                                                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                                                    <span class="label">FOB Price: </span>
                                                                    <span class="value">US $300-450 </span>
                                                                    <span class="unit">/ Piece</span>
                                                                </div>
                                                            </div>
                                                            <div class="min_order" title="Min. Order: 1 Piece">
                                                                <span class="label">Min. Order: </span>
                                                                <span class="value">1 Piece</span>
                                                            </div>


                                                            <div class="finshlinks">
                                                                <ul>
                                                                    <li>
                                                                        <p class="text-muted mt-3">
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star "></i>
                                                                            <i class="bx bxs-star"></i>
                                                                        </p>
                                                                    </li>
                                                                    <li class="addbuton">
                                                                        <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                                                            <i class="bx bx-basket"></i>
                                                                        </button>
                                                                    </li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                    </div>
                                                      
                                                
                                                </div>
                                                
                                            </div>
                                        </div>
                                        
                                    </div>
                                </div>
                                
                
                                                                <div class="col-lg-12" style="display: none">
                                    <div class="tapsProduct">
                                         <!-- Nav tabs -->
                                        <ul class="nav nav-pills " role="tablist">
                                            <li class="nav-item waves-effect waves-light">
                                                <a class="nav-link active" data-bs-toggle="tab" href="#home-1" role="tab">
                                                   
                                                    <span class=" d-sm-block">TV & Video</span> 
                                                </a>
                                            </li>
                                            <li class="nav-item waves-effect waves-light">
                                                <a class="nav-link" data-bs-toggle="tab" href="#profile-1" role="tab">
                                                   
                                                    <span class=" d-sm-block">Audio & Theater</span> 
                                                </a>
                                            </li>
                                            <li class="nav-item waves-effect waves-light">
                                                <a class="nav-link" data-bs-toggle="tab" href="#messages-1" role="tab">
                                                    
                                                    <span class="d-sm-block">Camera, Photo</span>   
                                                </a>
                                            </li>
                                            <li class="nav-item waves-effect waves-light">
                                                <a class="nav-link" data-bs-toggle="tab" href="#settings-1" role="tab">
                                                    
                                                    <span class=" d-sm-block"> Accessories</span>    
                                                </a>
                                            </li>
                                           
                                        </ul>
        
                                        <!-- Tab panes -->
                                        <div class="tab-content p-3 text-muted">
                                            <div class="tab-pane PopuProd active" id="home-1" role="tabpanel">
                                                <div class="PopuProdtemsss productsss ">
                                                    <div class="row">
                                                        <div class="col-lg-3 col-md-6 col-sm-12 col-xs-12 othercoool">
                                                              <div class="PopuProdItem">
                                                        <div class="imges">
                                                            <img src="{{asset('frontUI/assets/img/cladreximg/Product/26.png')}}">
                                                        </div>
                                                        <div class="buttonsssRight">
                                                            <ul>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-heart"></i>
                                                                    </a>
                                                                </li>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-sync"></i>
                                                                    </a>
                                                                </li>
                                                                
                                                            </ul>
                                                        </div>
                                                        <div class="details">
                                                            <div class="categoru">
                                                                <a href="#" class="catlinl">zara official</a>
                                                            </div>
                                                            <div class="nameprod">
                                                                <a href="NewProductdetails.html" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                                                            </div>
                                                            <div class="price">
                                                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                                                    <span class="label">FOB Price: </span>
                                                                    <span class="value">US $300-450 </span>
                                                                    <span class="unit">/ Piece</span>
                                                                </div>
                                                            </div>
                                                            <div class="min_order" title="Min. Order: 1 Piece">
                                                                <span class="label">Min. Order: </span>
                                                                <span class="value">1 Piece</span>
                                                            </div>


                                                            <div class="finshlinks">
                                                                <ul>
                                                                    <li>
                                                                        <p class="text-muted mt-3">
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star"></i>
                                                                        </p>
                                                                    </li>
                                                                    <li class="addbuton">
                                                                        <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                                    <i class="bx bx-basket"></i>
                                                </button>
                                                                    </li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                    </div>
                                                  
                                                        </div>
                                                        <div class="col-lg-3 col-md-6 col-sm-12 col-xs-12 othercoool">
   <div class="PopuProdItem">
                                                        <div class="imges">
                                                            <img src="{{asset('frontUI/assets/img/cladreximg/Product/27.png')}}">
                                                        </div>
                                                        <div class="buttonsssRight">
                                                            <ul>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-heart"></i>
                                                                    </a>
                                                                </li>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-sync"></i>
                                                                    </a>
                                                                </li>
                                                                
                                                            </ul>
                                                        </div>
                                                        <div class="details">
                                                            <div class="categoru">
                                                                <a href="#" class="catlinl">zara official</a>
                                                            </div>
                                                            <div class="nameprod">
                                                                <a href="NewProductdetails.html" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                                                            </div>
                                                            <div class="price">
                                                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                                                    <span class="label">FOB Price: </span>
                                                                    <span class="value">US $300-450 </span>
                                                                    <span class="unit">/ Piece</span>
                                                                </div>
                                                            </div>
                                                            <div class="min_order" title="Min. Order: 1 Piece">
                                                                <span class="label">Min. Order: </span>
                                                                <span class="value">1 Piece</span>
                                                            </div>


                                                            <div class="finshlinks">
                                                                <ul>
                                                                    <li>
                                                                        <p class="text-muted mt-3">
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star"></i>
                                                                        </p>
                                                                    </li>
                                                                    <li class="addbuton">
                                                                        <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                                    <i class="bx bx-basket"></i>
                                                </button>
                                                                    </li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                    </div>
                                                  
                                                        </div>
                                                        <div class="col-lg-3 col-md-6 col-sm-12 col-xs-12 othercoool">
   <div class="PopuProdItem">
                                                        <div class="imges">
                                                            <img src="{{asset('frontUI/assets/img/cladreximg/Product/28.png')}}">
                                                        </div>
                                                        <div class="buttonsssRight">
                                                            <ul>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-heart"></i>
                                                                    </a>
                                                                </li>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-sync"></i>
                                                                    </a>
                                                                </li>
                                                                
                                                            </ul>
                                                        </div>
                                                        <div class="details">
                                                            <div class="categoru">
                                                                <a href="#" class="catlinl">zara official</a>
                                                            </div>
                                                            <div class="nameprod">
                                                                <a href="NewProductdetails.html" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                                                            </div>
                                                            <div class="price">
                                                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                                                    <span class="label">FOB Price: </span>
                                                                    <span class="value">US $300-450 </span>
                                                                    <span class="unit">/ Piece</span>
                                                                </div>
                                                            </div>
                                                            <div class="min_order" title="Min. Order: 1 Piece">
                                                                <span class="label">Min. Order: </span>
                                                                <span class="value">1 Piece</span>
                                                            </div>


                                                            <div class="finshlinks">
                                                                <ul>
                                                                    <li>
                                                                        <p class="text-muted mt-3">
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star"></i>
                                                                        </p>
                                                                    </li>
                                                                    <li class="addbuton">
                                                                        <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                                    <i class="bx bx-basket"></i>
                                                </button>
                                                                    </li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                    </div>
                                                  
                                                        </div>
                                                        
                                                          <div class="col-lg-3 col-md-6 col-sm-12 col-xs-12 othercoool">
   <div class="PopuProdItem">
                                                        <div class="imges">
                                                            <img src="{{asset('frontUI/assets/img/cladreximg/Product/29.png')}}">
                                                        </div>
                                                        <div class="buttonsssRight">
                                                            <ul>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-heart"></i>
                                                                    </a>
                                                                </li>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-sync"></i>
                                                                    </a>
                                                                </li>
                                                                
                                                            </ul>
                                                        </div>
                                                        <div class="details">
                                                            <div class="categoru">
                                                                <a href="#" class="catlinl">zara official</a>
                                                            </div>
                                                            <div class="nameprod">
                                                                <a href="NewProductdetails.html" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                                                            </div>
                                                            <div class="price">
                                                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                                                    <span class="label">FOB Price: </span>
                                                                    <span class="value">US $300-450 </span>
                                                                    <span class="unit">/ Piece</span>
                                                                </div>
                                                            </div>
                                                            <div class="min_order" title="Min. Order: 1 Piece">
                                                                <span class="label">Min. Order: </span>
                                                                <span class="value">1 Piece</span>
                                                            </div>


                                                            <div class="finshlinks">
                                                                <ul>
                                                                    <li>
                                                                        <p class="text-muted mt-3">
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star"></i>
                                                                        </p>
                                                                    </li>
                                                                    <li class="addbuton">
                                                                        <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                                    <i class="bx bx-basket"></i>
                                                </button>
                                                                    </li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                    </div>
                                                  
                                                        </div>
                                                        
                                                        
                                                    </div>
                                                </div>
                                                
                                            </div>
                                            <div class="tab-pane PopuProd" id="profile-1" role="tabpanel">
                                                <div class="PopuProdtemsss productsss ">
                                                    <div class="row">
                                                        <div class="col-lg-3 col-md-6 col-sm-12 col-xs-12 othercoool">
                                                              <div class="PopuProdItem">
                                                        <div class="imges">
                                                            <img src="{{asset('frontUI/assets/img/cladreximg/Product/27.png')}}">
                                                        </div>
                                                        <div class="buttonsssRight">
                                                            <ul>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-heart"></i>
                                                                    </a>
                                                                </li>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-sync"></i>
                                                                    </a>
                                                                </li>
                                                                
                                                            </ul>
                                                        </div>
                                                        <div class="details">
                                                            <div class="categoru">
                                                                <a href="#" class="catlinl">zara official</a>
                                                            </div>
                                                            <div class="nameprod">
                                                                <a href="NewProductdetails.html" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                                                            </div>
                                                            <div class="price">
                                                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                                                    <span class="label">FOB Price: </span>
                                                                    <span class="value">US $300-450 </span>
                                                                    <span class="unit">/ Piece</span>
                                                                </div>
                                                            </div>
                                                            <div class="min_order" title="Min. Order: 1 Piece">
                                                                <span class="label">Min. Order: </span>
                                                                <span class="value">1 Piece</span>
                                                            </div>


                                                            <div class="finshlinks">
                                                                <ul>
                                                                    <li>
                                                                        <p class="text-muted mt-3">
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star"></i>
                                                                        </p>
                                                                    </li>
                                                                    <li class="addbuton">
                                                                        <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                                    <i class="bx bx-basket"></i>
                                                </button>
                                                                    </li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                    </div>
                                                  
                                                        </div>
                                                        <div class="col-lg-3 col-md-6 col-sm-12 col-xs-12 othercoool">
   <div class="PopuProdItem">
                                                        <div class="imges">
                                                            <img src="{{asset('frontUI/assets/img/cladreximg/Product/28.png')}}">
                                                        </div>
                                                        <div class="buttonsssRight">
                                                            <ul>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-heart"></i>
                                                                    </a>
                                                                </li>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-sync"></i>
                                                                    </a>
                                                                </li>
                                                                
                                                            </ul>
                                                        </div>
                                                        <div class="details">
                                                            <div class="categoru">
                                                                <a href="#" class="catlinl">zara official</a>
                                                            </div>
                                                            <div class="nameprod">
                                                                <a href="NewProductdetails.html" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                                                            </div>
                                                            <div class="price">
                                                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                                                    <span class="label">FOB Price: </span>
                                                                    <span class="value">US $300-450 </span>
                                                                    <span class="unit">/ Piece</span>
                                                                </div>
                                                            </div>
                                                            <div class="min_order" title="Min. Order: 1 Piece">
                                                                <span class="label">Min. Order: </span>
                                                                <span class="value">1 Piece</span>
                                                            </div>


                                                            <div class="finshlinks">
                                                                <ul>
                                                                    <li>
                                                                        <p class="text-muted mt-3">
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star"></i>
                                                                        </p>
                                                                    </li>
                                                                    <li class="addbuton">
                                                                        <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                                    <i class="bx bx-basket"></i>
                                                </button>
                                                                    </li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                    </div>
                                                  
                                                        </div>
                                                        <div class="col-lg-3 col-md-6 col-sm-12 col-xs-12 othercoool">
   <div class="PopuProdItem">
                                                        <div class="imges">
                                                            <img src="{{asset('frontUI/assets/img/cladreximg/Product/26.png')}}">
                                                        </div>
                                                        <div class="buttonsssRight">
                                                            <ul>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-heart"></i>
                                                                    </a>
                                                                </li>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-sync"></i>
                                                                    </a>
                                                                </li>
                                                                
                                                            </ul>
                                                        </div>
                                                        <div class="details">
                                                            <div class="categoru">
                                                                <a href="#" class="catlinl">zara official</a>
                                                            </div>
                                                            <div class="nameprod">
                                                                <a href="NewProductdetails.html" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                                                            </div>
                                                            <div class="price">
                                                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                                                    <span class="label">FOB Price: </span>
                                                                    <span class="value">US $300-450 </span>
                                                                    <span class="unit">/ Piece</span>
                                                                </div>
                                                            </div>
                                                            <div class="min_order" title="Min. Order: 1 Piece">
                                                                <span class="label">Min. Order: </span>
                                                                <span class="value">1 Piece</span>
                                                            </div>


                                                            <div class="finshlinks">
                                                                <ul>
                                                                    <li>
                                                                        <p class="text-muted mt-3">
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star"></i>
                                                                        </p>
                                                                    </li>
                                                                    <li class="addbuton">
                                                                        <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                                    <i class="bx bx-basket"></i>
                                                </button>
                                                                    </li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                    </div>
                                                  
                                                        </div>
                                                        
                                                          <div class="col-lg-3 col-md-6 col-sm-12 col-xs-12 othercoool">
   <div class="PopuProdItem">
                                                        <div class="imges">
                                                            <img src="{{asset('frontUI/assets/img/cladreximg/Product/29.png')}}">
                                                        </div>
                                                        <div class="buttonsssRight">
                                                            <ul>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-heart"></i>
                                                                    </a>
                                                                </li>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-sync"></i>
                                                                    </a>
                                                                </li>
                                                                
                                                            </ul>
                                                        </div>
                                                        <div class="details">
                                                            <div class="categoru">
                                                                <a href="#" class="catlinl">zara official</a>
                                                            </div>
                                                            <div class="nameprod">
                                                                <a href="NewProductdetails.html" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                                                            </div>
                                                            <div class="price">
                                                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                                                    <span class="label">FOB Price: </span>
                                                                    <span class="value">US $300-450 </span>
                                                                    <span class="unit">/ Piece</span>
                                                                </div>
                                                            </div>
                                                            <div class="min_order" title="Min. Order: 1 Piece">
                                                                <span class="label">Min. Order: </span>
                                                                <span class="value">1 Piece</span>
                                                            </div>


                                                            <div class="finshlinks">
                                                                <ul>
                                                                    <li>
                                                                        <p class="text-muted mt-3">
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star"></i>
                                                                        </p>
                                                                    </li>
                                                                    <li class="addbuton">
                                                                        <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                                    <i class="bx bx-basket"></i>
                                                </button>
                                                                    </li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                    </div>
                                                  
                                                        </div>
                                                        
                                                        
                                                    </div>
                                                </div>
                                                
                                            </div>
                                            <div class="tab-pane PopuProd" id="messages-1" role="tabpanel">
                                                <div class="PopuProdtemsss productsss ">
                                                    <div class="row">
                                                        <div class="col-lg-3 col-md-6 col-sm-12 col-xs-12 othercoool">
                                                              <div class="PopuProdItem">
                                                        <div class="imges">
                                                            <img src="{{asset('frontUI/assets/img/cladreximg/Product/26.png')}}">
                                                        </div>
                                                        <div class="buttonsssRight">
                                                            <ul>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-heart"></i>
                                                                    </a>
                                                                </li>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-sync"></i>
                                                                    </a>
                                                                </li>
                                                                
                                                            </ul>
                                                        </div>
                                                        <div class="details">
                                                            <div class="categoru">
                                                                <a href="#" class="catlinl">zara official</a>
                                                            </div>
                                                            <div class="nameprod">
                                                                <a href="NewProductdetails.html" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                                                            </div>
                                                            <div class="price">
                                                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                                                    <span class="label">FOB Price: </span>
                                                                    <span class="value">US $300-450 </span>
                                                                    <span class="unit">/ Piece</span>
                                                                </div>
                                                            </div>
                                                            <div class="min_order" title="Min. Order: 1 Piece">
                                                                <span class="label">Min. Order: </span>
                                                                <span class="value">1 Piece</span>
                                                            </div>


                                                            <div class="finshlinks">
                                                                <ul>
                                                                    <li>
                                                                        <p class="text-muted mt-3">
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star"></i>
                                                                        </p>
                                                                    </li>
                                                                    <li class="addbuton">
                                                                        <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                                    <i class="bx bx-basket"></i>
                                                </button>
                                                                    </li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                    </div>
                                                  
                                                        </div>
                                                        <div class="col-lg-3 col-md-6 col-sm-12 col-xs-12 othercoool">
   <div class="PopuProdItem">
                                                        <div class="imges">
                                                            <img src="{{asset('frontUI/assets/img/cladreximg/Product/27.png')}}">
                                                        </div>
                                                        <div class="buttonsssRight">
                                                            <ul>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-heart"></i>
                                                                    </a>
                                                                </li>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-sync"></i>
                                                                    </a>
                                                                </li>
                                                                
                                                            </ul>
                                                        </div>
                                                        <div class="details">
                                                            <div class="categoru">
                                                                <a href="#" class="catlinl">zara official</a>
                                                            </div>
                                                            <div class="nameprod">
                                                                <a href="NewProductdetails.html" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                                                            </div>
                                                            <div class="price">
                                                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                                                    <span class="label">FOB Price: </span>
                                                                    <span class="value">US $300-450 </span>
                                                                    <span class="unit">/ Piece</span>
                                                                </div>
                                                            </div>
                                                            <div class="min_order" title="Min. Order: 1 Piece">
                                                                <span class="label">Min. Order: </span>
                                                                <span class="value">1 Piece</span>
                                                            </div>


                                                            <div class="finshlinks">
                                                                <ul>
                                                                    <li>
                                                                        <p class="text-muted mt-3">
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star"></i>
                                                                        </p>
                                                                    </li>
                                                                    <li class="addbuton">
                                                                        <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                                    <i class="bx bx-basket"></i>
                                                </button>
                                                                    </li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                    </div>
                                                  
                                                        </div>
                                                        <div class="col-lg-3 col-md-6 col-sm-12 col-xs-12 othercoool">
   <div class="PopuProdItem">
                                                        <div class="imges">
                                                            <img src="{{asset('frontUI/assets/img/cladreximg/Product/28.png')}}">
                                                        </div>
                                                        <div class="buttonsssRight">
                                                            <ul>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-heart"></i>
                                                                    </a>
                                                                </li>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-sync"></i>
                                                                    </a>
                                                                </li>
                                                                
                                                            </ul>
                                                        </div>
                                                        <div class="details">
                                                            <div class="categoru">
                                                                <a href="#" class="catlinl">zara official</a>
                                                            </div>
                                                            <div class="nameprod">
                                                                <a href="NewProductdetails.html" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                                                            </div>
                                                            <div class="price">
                                                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                                                    <span class="label">FOB Price: </span>
                                                                    <span class="value">US $300-450 </span>
                                                                    <span class="unit">/ Piece</span>
                                                                </div>
                                                            </div>
                                                            <div class="min_order" title="Min. Order: 1 Piece">
                                                                <span class="label">Min. Order: </span>
                                                                <span class="value">1 Piece</span>
                                                            </div>


                                                            <div class="finshlinks">
                                                                <ul>
                                                                    <li>
                                                                        <p class="text-muted mt-3">
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star"></i>
                                                                        </p>
                                                                    </li>
                                                                    <li class="addbuton">
                                                                        <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                                    <i class="bx bx-basket"></i>
                                                </button>
                                                                    </li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                    </div>
                                                  
                                                        </div>
                                                        
                                                          <div class="col-lg-3 col-md-6 col-sm-12 col-xs-12 othercoool">
   <div class="PopuProdItem">
                                                        <div class="imges">
                                                            <img src="{{asset('frontUI/assets/img/cladreximg/Product/29.png')}}">
                                                        </div>
                                                        <div class="buttonsssRight">
                                                            <ul>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-heart"></i>
                                                                    </a>
                                                                </li>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-sync"></i>
                                                                    </a>
                                                                </li>
                                                                
                                                            </ul>
                                                        </div>
                                                        <div class="details">
                                                            <div class="categoru">
                                                                <a href="#" class="catlinl">zara official</a>
                                                            </div>
                                                            <div class="nameprod">
                                                                <a href="NewProductdetails.html" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                                                            </div>
                                                            <div class="price">
                                                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                                                    <span class="label">FOB Price: </span>
                                                                    <span class="value">US $300-450 </span>
                                                                    <span class="unit">/ Piece</span>
                                                                </div>
                                                            </div>
                                                            <div class="min_order" title="Min. Order: 1 Piece">
                                                                <span class="label">Min. Order: </span>
                                                                <span class="value">1 Piece</span>
                                                            </div>


                                                            <div class="finshlinks">
                                                                <ul>
                                                                    <li>
                                                                        <p class="text-muted mt-3">
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star"></i>
                                                                        </p>
                                                                    </li>
                                                                    <li class="addbuton">
                                                                        <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                                    <i class="bx bx-basket"></i>
                                                </button>
                                                                    </li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                    </div>
                                                  
                                                        </div>
                                                        
                                                        
                                                    </div>
                                                </div>
                                                
                                            </div>
                                            <div class="tab-pane PopuProd" id="settings-1" role="tabpanel">
                                                <div class="PopuProdtemsss productsss ">
                                                    <div class="row">
                                                        <div class="col-lg-3 col-md-6 col-sm-12 col-xs-12 othercoool">
                                                              <div class="PopuProdItem">
                                                        <div class="imges">
                                                            <img src="{{asset('frontUI/assets/img/cladreximg/Product/26.png')}}">
                                                        </div>
                                                        <div class="buttonsssRight">
                                                            <ul>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-heart"></i>
                                                                    </a>
                                                                </li>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-sync"></i>
                                                                    </a>
                                                                </li>
                                                                
                                                            </ul>
                                                        </div>
                                                        <div class="details">
                                                            <div class="categoru">
                                                                <a href="#" class="catlinl">zara official</a>
                                                            </div>
                                                            <div class="nameprod">
                                                                <a href="NewProductdetails.html" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                                                            </div>
                                                            <div class="price">
                                                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                                                    <span class="label">FOB Price: </span>
                                                                    <span class="value">US $300-450 </span>
                                                                    <span class="unit">/ Piece</span>
                                                                </div>
                                                            </div>
                                                            <div class="min_order" title="Min. Order: 1 Piece">
                                                                <span class="label">Min. Order: </span>
                                                                <span class="value">1 Piece</span>
                                                            </div>


                                                            <div class="finshlinks">
                                                                <ul>
                                                                    <li>
                                                                        <p class="text-muted mt-3">
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star"></i>
                                                                        </p>
                                                                    </li>
                                                                    <li class="addbuton">
                                                                        <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                                    <i class="bx bx-basket"></i>
                                                </button>
                                                                    </li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                    </div>
                                                  
                                                        </div>
                                                        <div class="col-lg-3 col-md-6 col-sm-12 col-xs-12 othercoool">
                                                              <div class="PopuProdItem">
                                                        <div class="imges">
                                                            <img src="{{asset('frontUI/assets/img/cladreximg/Product/27.png')}}">
                                                        </div>
                                                        <div class="buttonsssRight">
                                                            <ul>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-heart"></i>
                                                                    </a>
                                                                </li>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-sync"></i>
                                                                    </a>
                                                                </li>
                                                                
                                                            </ul>
                                                        </div>
                                                        <div class="details">
                                                            <div class="categoru">
                                                                <a href="#" class="catlinl">zara official</a>
                                                            </div>
                                                            <div class="nameprod">
                                                                <a href="NewProductdetails.html" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                                                            </div>
                                                            <div class="price">
                                                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                                                    <span class="label">FOB Price: </span>
                                                                    <span class="value">US $300-450 </span>
                                                                    <span class="unit">/ Piece</span>
                                                                </div>
                                                            </div>
                                                            <div class="min_order" title="Min. Order: 1 Piece">
                                                                <span class="label">Min. Order: </span>
                                                                <span class="value">1 Piece</span>
                                                            </div>


                                                            <div class="finshlinks">
                                                                <ul>
                                                                    <li>
                                                                        <p class="text-muted mt-3">
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star"></i>
                                                                        </p>
                                                                    </li>
                                                                    <li class="addbuton">
                                                                        <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                                    <i class="bx bx-basket"></i>
                                                </button>
                                                                    </li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                    </div>
                                                  
                                                        </div>
                                                        <div class="col-lg-3 col-md-6 col-sm-12 col-xs-12 othercoool">
                                                                <div class="PopuProdItem">
                                                        <div class="imges">
                                                            <img src="{{asset('frontUI/assets/img/cladreximg/Product/28.png')}}">
                                                        </div>
                                                        <div class="buttonsssRight">
                                                            <ul>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-heart"></i>
                                                                    </a>
                                                                </li>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-sync"></i>
                                                                    </a>
                                                                </li>
                                                                
                                                            </ul>
                                                        </div>
                                                        <div class="details">
                                                            <div class="categoru">
                                                                <a href="#" class="catlinl">zara official</a>
                                                            </div>
                                                            <div class="nameprod">
                                                                <a href="NewProductdetails.html" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                                                            </div>
                                                            <div class="price">
                                                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                                                    <span class="label">FOB Price: </span>
                                                                    <span class="value">US $300-450 </span>
                                                                    <span class="unit">/ Piece</span>
                                                                </div>
                                                            </div>
                                                            <div class="min_order" title="Min. Order: 1 Piece">
                                                                <span class="label">Min. Order: </span>
                                                                <span class="value">1 Piece</span>
                                                            </div>


                                                            <div class="finshlinks">
                                                                <ul>
                                                                    <li>
                                                                        <p class="text-muted mt-3">
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star"></i>
                                                                        </p>
                                                                    </li>
                                                                    <li class="addbuton">
                                                                        <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                                    <i class="bx bx-basket"></i>
                                                </button>
                                                                    </li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                    </div>
                                                  
                                                        </div>
                                                        
                                                          <div class="col-lg-3 col-md-6 col-sm-12 col-xs-12 othercoool">
                                                        <div class="PopuProdItem">
                                                        <div class="imges">
                                                            <img src="{{asset('frontUI/assets/img/cladreximg/Product/29.png')}}">
                                                        </div>
                                                        <div class="buttonsssRight">
                                                            <ul>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-heart"></i>
                                                                    </a>
                                                                </li>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-sync"></i>
                                                                    </a>
                                                                </li>
                                                                
                                                            </ul>
                                                        </div>
                                                        <div class="details">
                                                            <div class="categoru">
                                                                <a href="#" class="catlinl">zara official</a>
                                                            </div>
                                                            <div class="nameprod">
                                                                <a href="NewProductdetails.html" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                                                            </div>
                                                            <div class="price">
                                                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                                                    <span class="label">FOB Price: </span>
                                                                    <span class="value">US $300-450 </span>
                                                                    <span class="unit">/ Piece</span>
                                                                </div>
                                                            </div>
                                                            <div class="min_order" title="Min. Order: 1 Piece">
                                                                <span class="label">Min. Order: </span>
                                                                <span class="value">1 Piece</span>
                                                            </div>


                                                            <div class="finshlinks">
                                                                <ul>
                                                                    <li>
                                                                        <p class="text-muted mt-3">
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star"></i>
                                                                        </p>
                                                                    </li>
                                                                    <li class="addbuton">
                                                                        <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                                    <i class="bx bx-basket"></i>
                                                </button>
                                                                    </li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                    </div>
                                                  
                                                        </div>
                                                        
                                                        
                                                    </div>
                                                </div>
                                                
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                
                
                
                
                
                
                
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="display: none">
                  <div class="addside">
                    <div class="addcontent">
                      <p>Adds</p>
                    </div>
                  </div>
                </div>
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12"  style="display: none">
                  <div class="services">
                    <div class="servcont">
                      <div class="title">
                        <h2 class="Sertittl">Our Services</h2>
                        <div class="line"></div>
                      </div>
                      <div class="servitemsss">
                        <div class="seritem">
                          <a href="" class="servitemcont">
                            <span class="linkcont">
                              <i class=" icofont icofont-5-star-hotel"></i>
                              <span>Hotels</span>
                            </span>
                          </a>
                        </div>
                        <div class="seritem">
                          <a href="" class="servitemcont">
                            <span class="linkcont">
                              <i class=" icofont icofont-plane-ticket"></i>
                              <span>Airlines</span>
                            </span>
                          </a>
                        </div>
                        <div class="seritem">
                          <a href="#" class="servitemcont itembig">
                            <span class="linkcont">
                              <i class=" icofont icofont-cart"></i>
                              <span>Shopping</span>
                            </span>
                          </a>
                        </div>
                        <div class="seritem">
                          <a href="" class="servitemcont">
                            <span class="linkcont">
                              <i class=" icofont icofont-open-eye"></i>
                              <span>Fairs</span>
                            </span>
                          </a>
                        </div>
                        <div class="seritem">
                          <a href="" class="servitemcont">
                            <span class="linkcont">
                              <i class=" icofont icofont-delivery-time"></i>
                              <span>Shipping</span>
                            </span>
                          </a>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>

                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12"  style="display: none">
                  <div class="Category">
                    <div class="Categorycont">
                      <div class="title">
                        <h2 class="Categorytittl">Categories</h2>
                        <div class="line"></div>
                      </div>
                      <div class="Categorytemsss">
                        <div class="row">
                                @if($categories)
                                  @foreach($categories as $category)
                          <div class="col-lg-3 col-md-3 col-sm-4 col-xs-6">
                            <div class="CategoryItem">
                              <a href="#" class="Catitdet">
                                <i class="fa fas {{$category->image}}"></i>
                                <span>{{$category->name}}</span>
                              </a>
                            </div>
                          </div>
                          @endforeach
                          @endif
                          <!--<div class="col-lg-3 col-md-3 col-sm-4 col-xs-6">-->
                          <!--  <div class="CategoryItem">-->
                          <!--    <a href="#" class="Catitdet">-->
                          <!--      <i class="fa fas fa-icons"></i>-->
                          <!--      <span>Bags</span>-->
                          <!--    </a>-->
                          <!--  </div>-->
                          <!--</div>-->
                          <!--<div class="col-lg-3 col-md-3 col-sm-4 col-xs-6">-->
                          <!--  <div class="CategoryItem">-->
                          <!--    <a href="#" class="Catitdet">-->
                          <!--      <i class="fa fas fa-icons"></i>-->
                          <!--      <span>Programs</span>-->
                          <!--    </a>-->
                          <!--  </div>-->
                          <!--</div>-->
                          <!--<div class="col-lg-3 col-md-3 col-sm-4 col-xs-6">-->
                          <!--  <div class="CategoryItem">-->
                          <!--    <a href="#" class="Catitdet">-->
                          <!--      <i class="fa fas fa-icons"></i>-->
                          <!--      <span>Children</span>-->
                          <!--    </a>-->
                          <!--  </div>-->
                          <!--</div>-->
                          <!--<div class="col-lg-3 col-md-3 col-sm-4 col-xs-6">-->
                          <!--  <div class="CategoryItem">-->
                          <!--    <a href="#" class="Catitdet">-->
                          <!--      <i class="fa fas fa-icons"></i>-->
                          <!--      <span>Garden</span>-->
                          <!--    </a>-->
                          <!--  </div>-->
                          <!--</div>-->
                          <!--<div class="col-lg-3 col-md-3 col-sm-4 col-xs-6">-->
                          <!--  <div class="CategoryItem">-->
                          <!--    <a href="#" class="Catitdet">-->
                          <!--      <i class="fa fas fa-icons"></i>-->
                          <!--      <span>Mobile & Accessories</span>-->
                          <!--    </a>-->
                          <!--  </div>-->
                          <!--</div>-->
                          <!--<div class="col-lg-3 col-md-3 col-sm-4 col-xs-6">-->
                          <!--  <div class="CategoryItem">-->
                          <!--    <a href="#" class="Catitdet">-->
                          <!--      <i class="fa fas fa-icons"></i>-->
                          <!--      <span>Movies & Music</span>-->
                          <!--    </a>-->
                          <!--  </div>-->
                          <!--</div>-->
                          <!--<div class="col-lg-3 col-md-3 col-sm-4 col-xs-6">-->
                          <!--  <div class="CategoryItem">-->
                          <!--    <a href="#" class="Catitdet">-->
                          <!--      <i class="fa fas fa-icons"></i>-->
                          <!--      <span>Food & Beverage</span>-->
                          <!--    </a>-->
                          <!--  </div>-->
                          <!--</div>-->
                        </div>
                        <div class="seemore">
                          <a href="#" class="seemorelink">
                            see more <span><i class="fas fa-chevron-down"></i></span>
                          </a>
                        </div>
                      </div>

                    </div>
                  </div>
                </div>
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12"  style="display: none">
                  <div class="PopuProd">
                    <div class="title">
                      <h2 class="PopuProdtittl">Popular Products</h2>
                      <div class="line"></div>
                    </div>
                    <div class="PopuProdtemsss">
                      <div class="row">


                        <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
                          <div class="PopuProdItem">
                            <div class="imges">
                              <img src="{{asset('frontUI/assets/img/cladreximg/papa.png')}}">
                            </div>
                            <div class="details">
                              <div class="categoru">
                                <a href="#" class="catlinl">zara official</a>
                              </div>
                              <div class="nameprod">
                                <a href="#" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                              </div>
                              <div class="price">
                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                  <span class="label">FOB Price: </span>
                                  <span class="value">US $300-450 </span>
                                  <span class="unit">/ Piece</span>
                                </div>
                              </div>
                              <div class="min_order" title="Min. Order: 1 Piece">
                                <span class="label">Min. Order: </span>
                                <span class="value">1 Piece</span>
                              </div>


                              <div class="finshlinks">
                                <ul>
                                  <li>
                                    <a href="#" class="wish">
                                      <i class="icofont icofont-heart"></i>
                                    </a>
                                  </li>
                                  <li>
                                    <a href="#" class="wish">
                                      <i class="icofont icofont-chart-bar-graph"></i>
                                    </a>
                                  </li>
                                  <li class="addbuton">
                                    <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                      <i class="bx bx-basket"></i>
                                    </button>
                                  </li>
                                </ul>
                              </div>
                            </div>
                          </div>
                        </div>
                        <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
                          <div class="PopuProdItem">
                            <div class="imges">
                              <img src="{{asset('frontUI/assets/img/cladreximg/papa.png')}}">
                            </div>
                            <div class="details">
                              <div class="categoru">
                                <a href="#" class="catlinl">zara official</a>
                              </div>
                              <div class="nameprod">
                                <a href="#" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                              </div>
                              <div class="price">
                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                  <span class="label">FOB Price: </span>
                                  <span class="value">US $300-450 </span>
                                  <span class="unit">/ Piece</span>
                                </div>
                              </div>
                              <div class="min_order" title="Min. Order: 1 Piece">
                                <span class="label">Min. Order: </span>
                                <span class="value">1 Piece</span>
                              </div>


                              <div class="finshlinks">
                                <ul>
                                  <li>
                                    <a href="#" class="wish">
                                      <i class="icofont icofont-heart"></i>
                                    </a>
                                  </li>
                                  <li>
                                    <a href="#" class="wish">
                                      <i class="icofont icofont-chart-bar-graph"></i>
                                    </a>
                                  </li>
                                  <li class="addbuton">
                                    <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                      <i class="bx bx-basket"></i>
                                    </button>
                                  </li>
                                </ul>
                              </div>
                            </div>
                          </div>
                        </div>

                        <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
                          <div class="PopuProdItem">
                            <div class="imges">
                              <img src="{{asset('frontUI/assets/img/cladreximg/papa.png')}}">
                            </div>
                            <div class="details">
                              <div class="categoru">
                                <a href="#" class="catlinl">zara official</a>
                              </div>
                              <div class="nameprod">
                                <a href="#" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                              </div>
                              <div class="price">
                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                  <span class="label">FOB Price: </span>
                                  <span class="value">US $300-450 </span>
                                  <span class="unit">/ Piece</span>
                                </div>
                              </div>
                              <div class="min_order" title="Min. Order: 1 Piece">
                                <span class="label">Min. Order: </span>
                                <span class="value">1 Piece</span>
                              </div>


                              <div class="finshlinks">
                                <ul>
                                  <li>
                                    <a href="#" class="wish">
                                      <i class="icofont icofont-heart"></i>
                                    </a>
                                  </li>
                                  <li>
                                    <a href="#" class="wish">
                                      <i class="icofont icofont-chart-bar-graph"></i>
                                    </a>
                                  </li>
                                  <li class="addbuton">
                                    <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                      <i class="bx bx-basket"></i>
                                    </button>
                                  </li>
                                </ul>
                              </div>
                            </div>
                          </div>
                        </div>


                      </div>
                      <div class="seemore">
                        <a href="#" class="seemorelink">
                          see more <span><i class="fas fa-chevron-down"></i></span>
                        </a>
                      </div>
                    </div>

                  </div>
                </div>

                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12"  style="display: none">
                  <div class="mediaphoto">
                    <div class="row">
                      <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                        <div class="mediaitem">
                          <a href="#" class="imgesssa">
                            <img src="{{asset('frontUI/assets/img/cladreximg/media1.png')}}">
                          </a>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
     <div class="col-lg-12"  style="display: none">
                                    <div class="productOwl PopuProd">
                                        <div class="title">
                                            <h2>Electronic <span>Product</span></h2>
                                        </div>
                                        <div class="Products PopuProdtemsss">
                                            <div class="owl-carousel owl-theme prprprOwl">
                                                <div class="item">
                                                <div class="PopuProdItem">
                                                        <div class="imges">
                                                            <img src="{{asset('frontUI/assets/img/cladreximg/Product/26.png')}}">
                                                        </div>
                                                        <div class="buttonsssRight">
                                                            <ul>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-heart"></i>
                                                                    </a>
                                                                </li>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-sync"></i>
                                                                    </a>
                                                                </li>
                                                                
                                                            </ul>
                                                        </div>
                                                        <div class="details">
                                                            <div class="categoru">
                                                                <a href="#" class="catlinl">zara official</a>
                                                            </div>
                                                            <div class="nameprod">
                                                                <a href="NewProductdetails.html" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                                                            </div>
                                                            <div class="price">
                                                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                                                    <!--<span class="label">FOB Price: </span>-->
                                                                    <span class="value">US $300-450 </span>
                                                                    <!--<span class="unit">/ Piece</span>-->
                                                                </div>
                                                            </div>
                                                            <div class="min_order" title="Min. Order: 1 Piece">
                                                                <span class="label">Min. Order: </span>
                                                                <span class="value">1 Piece</span>
                                                            </div>


                                                            <div class="finshlinks">
                                                                <ul>
                                                                    <li>
                                                                        <p class="text-muted mt-3">
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star"></i>
                                                                        </p>
                                                                    </li>
                                                                    
                                                                    <li class="addbuton">
                                                                        <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                                    <i class="bx bx-basket"></i>
                                                </button>
                                                                    </li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                    </div>
                                                      
                                                
                                                </div>
                                                <div class="item">
                                                <div class="PopuProdItem">
                                                        <div class="imges">
                                                            <img src="{{asset('frontUI/assets/img/cladreximg/Product/27.png')}}">
                                                        </div>
                                                        <div class="buttonsssRight">
                                                            <ul>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-heart"></i>
                                                                    </a>
                                                                </li>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-sync"></i>
                                                                    </a>
                                                                </li>
                                                                
                                                            </ul>
                                                        </div>
                                                        <div class="details">
                                                            <div class="categoru">
                                                                <a href="#" class="catlinl">zara official</a>
                                                            </div>
                                                            <div class="nameprod">
                                                                <a href="NewProductdetails.html" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                                                            </div>
                                                            <div class="price">
                                                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                                                    <span class="label">FOB Price: </span>
                                                                    <span class="value">US $300-450 </span>
                                                                    <span class="unit">/ Piece</span>
                                                                </div>
                                                            </div>
                                                            <div class="min_order" title="Min. Order: 1 Piece">
                                                                <span class="label">Min. Order: </span>
                                                                <span class="value">1 Piece</span>
                                                            </div>


                                                            <div class="finshlinks">
                                                                <ul>
                                                                    <li>
                                                                        <p class="text-muted mt-3">
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star "></i>
                                                                            <i class="bx bxs-star"></i>
                                                                        </p>
                                                                    </li>
                                                                    <li class="addbuton">
                                                                        <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                                    <i class="bx bx-basket"></i>
                                                </button>
                                                                    </li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                    </div>
                                                      
                                                
                                                </div>
                                                
                                                <div class="item">
                                                <div class="PopuProdItem">
                                                        <div class="imges">
                                                            <img src="{{asset('frontUI/assets/img/cladreximg/Product/28.png')}}">
                                                        </div>
                                                        <div class="buttonsssRight">
                                                            <ul>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-heart"></i>
                                                                    </a>
                                                                </li>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-sync"></i>
                                                                    </a>
                                                                </li>
                                                                
                                                            </ul>
                                                        </div>
                                                        <div class="details">
                                                            <div class="categoru">
                                                                <a href="#" class="catlinl">zara official</a>
                                                            </div>
                                                            <div class="nameprod">
                                                                <a href="NewProductdetails.html" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                                                            </div>
                                                            <div class="price">
                                                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                                                    <span class="label">FOB Price: </span>
                                                                    <span class="value">US $300-450 </span>
                                                                    <span class="unit">/ Piece</span>
                                                                </div>
                                                            </div>
                                                            <div class="min_order" title="Min. Order: 1 Piece">
                                                                <span class="label">Min. Order: </span>
                                                                <span class="value">1 Piece</span>
                                                            </div>


                                                            <div class="finshlinks">
                                                                <ul>
                                                                    <li>
                                                                        <p class="text-muted mt-3">
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star "></i>
                                                                            <i class="bx bxs-star "></i>
                                                                            <i class="bx bxs-star"></i>
                                                                        </p>
                                                                    </li>
                                                                    <li class="addbuton">
                                                                        <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                                    <i class="bx bx-basket"></i>
                                                </button>
                                                                    </li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                    </div>
                                                      
                                                
                                                </div>
                                                
                                                <div class="item">
                                                <div class="PopuProdItem">
                                                        <div class="imges">
                                                            <img src="{{asset('frontUI/assets/img/cladreximg/Product/29.png')}}">
                                                        </div>
                                                        <div class="buttonsssRight">
                                                            <ul>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-heart"></i>
                                                                    </a>
                                                                </li>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-sync"></i>
                                                                    </a>
                                                                </li>
                                                                
                                                            </ul>
                                                        </div>
                                                        <div class="details">
                                                            <div class="categoru">
                                                                <a href="#" class="catlinl">zara official</a>
                                                            </div>
                                                            <div class="nameprod">
                                                                <a href="NewProductdetails.html" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                                                            </div>
                                                            <div class="price">
                                                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                                                    <span class="label">FOB Price: </span>
                                                                    <span class="value">US $300-450 </span>
                                                                    <span class="unit">/ Piece</span>
                                                                </div>
                                                            </div>
                                                            <div class="min_order" title="Min. Order: 1 Piece">
                                                                <span class="label">Min. Order: </span>
                                                                <span class="value">1 Piece</span>
                                                            </div>


                                                            <div class="finshlinks">
                                                                <ul>
                                                                    <li>
                                                                        <p class="text-muted mt-3">
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star"></i>
                                                                            <i class="bx bxs-star "></i>
                                                                            <i class="bx bxs-star"></i>
                                                                        </p>
                                                                    </li>
                                                                    <li class="addbuton">
                                                                        <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                                    <i class="bx bx-basket"></i>
                                                </button>
                                                                    </li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                    </div>
                                                      
                                                
                                                </div>
                                                
                                                <div class="item">
                                                <div class="PopuProdItem">
                                                        <div class="imges">
                                                            <img src="{{asset('frontUI/assets/img/cladreximg/Product/26.png')}}">
                                                        </div>
                                                        <div class="buttonsssRight">
                                                            <ul>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-heart"></i>
                                                                    </a>
                                                                </li>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-sync"></i>
                                                                    </a>
                                                                </li>
                                                                
                                                            </ul>
                                                        </div>
                                                        <div class="details">
                                                            <div class="categoru">
                                                                <a href="#" class="catlinl">zara official</a>
                                                            </div>
                                                            <div class="nameprod">
                                                                <a href="NewProductdetails.html" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                                                            </div>
                                                            <div class="price">
                                                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                                                    <span class="label">FOB Price: </span>
                                                                    <span class="value">US $300-450 </span>
                                                                    <span class="unit">/ Piece</span>
                                                                </div>
                                                            </div>
                                                            <div class="min_order" title="Min. Order: 1 Piece">
                                                                <span class="label">Min. Order: </span>
                                                                <span class="value">1 Piece</span>
                                                            </div>


                                                            <div class="finshlinks">
                                                                <ul>
                                                                    <li>
                                                                        <p class="text-muted mt-3">
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star"></i>
                                                                            <i class="bx bxs-star "></i>
                                                                            <i class="bx bxs-star"></i>
                                                                        </p>
                                                                    </li>
                                                                    <li class="addbuton">
                                                                        <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                                    <i class="bx bx-basket"></i>
                                                </button>
                                                                    </li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                    </div>
                                                      
                                                
                                                </div>
                                                
                                                <div class="item">
                                                <div class="PopuProdItem">
                                                        <div class="imges">
                                                            <img src="{{asset('frontUI/assets/img/cladreximg/Product/27.png')}}">
                                                        </div>
                                                        <div class="buttonsssRight">
                                                            <ul>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-heart"></i>
                                                                    </a>
                                                                </li>
                                                                <li>
                                                                    <a href="#">
                                                                        <i class="bx bx-sync"></i>
                                                                    </a>
                                                                </li>
                                                                
                                                            </ul>
                                                        </div>
                                                        <div class="details">
                                                            <div class="categoru">
                                                                <a href="#" class="catlinl">zara official</a>
                                                            </div>
                                                            <div class="nameprod">
                                                                <a href="NewProductdetails.html" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                                                            </div>
                                                            <div class="price">
                                                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                                                    <span class="label">FOB Price: </span>
                                                                    <span class="value">US $300-450 </span>
                                                                    <span class="unit">/ Piece</span>
                                                                </div>
                                                            </div>
                                                            <div class="min_order" title="Min. Order: 1 Piece">
                                                                <span class="label">Min. Order: </span>
                                                                <span class="value">1 Piece</span>
                                                            </div>


                                                            <div class="finshlinks">
                                                                <ul>
                                                                    <li>
                                                                        <p class="text-muted mt-3">
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star text-warning"></i>
                                                                            <i class="bx bxs-star "></i>
                                                                            <i class="bx bxs-star"></i>
                                                                        </p>
                                                                    </li>
                                                                    <li class="addbuton">
                                                                        <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                                                            <i class="bx bx-basket"></i>
                                                                        </button>
                                                                    </li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                    </div>
                                                      
                                                
                                                </div>
                                                
                                            </div>
                                        </div>
                                        
                                    </div>
                                </div>
                                                        
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="display: none">
                  <div class="hotdeals">
                    <div class="tittle">
                      <h2 class="PopuProdtittl">Popular Products</h2>
                      <div class="line"></div>
                    </div>
                    <div class="hotdealsCont">
                      <div class="row">
                        <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                          <div class="dealsitem">
                            <div class="row">
                              <div class="col-lg-7 col-md-7 col-sm-7 col-xs-12">
                                <div class="dealsdetail">
                                  <div class="name">
                                    <a href="#" class="dellinnam">Deal Of the Day</a>
                                  </div>
                                  <div class="deatlssss">
                                    <p class="limt">Limited Quantity Last</p>
                                    <h3 class="fully">Fully Skin Set Coat</h3>
                                    <div class="price">
                                      <p><span>$330</span> was $500</p>
                                    </div>
                                    <div class="shop">
                                      <a href="#" class="shoplink">Shop Now <span><i class="fas fa-long-arrow-alt-right"></i></span></a>
                                    </div>
                                  </div>
                                  <div class="timers">
                                    <ul>
                                      <li>
                                        <p class="pcont">05</p>
                                        <span class="punit">Hours</span>
                                      </li>
                                      <li>
                                        <p class="pcont">45</p>
                                        <span class="punit">Minutes</span>
                                      </li>
                                      <li>
                                        <p class="pcont">50</p>
                                        <span class="punit">Seconds</span>
                                      </li>
                                    </ul>
                                  </div>
                                </div>
                              </div>
                              <div class="col-lg-5 col-md-5 col-sm-5 col-xs-12">
                                <div class="Daelsimg">
                                  <img src="{{asset('frontUI/assets/img/cladreximg/deals1.jpg')}}">
                                </div>
                              </div>
                            </div>
                          </div>
                        </div>
                        <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                          <div class="dealsitem">
                            <div class="row">
                              <div class="col-lg-7 col-md-7 col-sm-7 col-xs-12">
                                <div class="dealsdetail">
                                  <div class="name">
                                    <a href="#" class="dellinnam">Deal Of the Day</a>
                                  </div>
                                  <div class="deatlssss">
                                    <p class="limt">Limited Quantity Last</p>
                                    <h3 class="fully">Fully Skin Set Coat</h3>
                                    <div class="price">
                                      <p><span>$330</span> was $500</p>
                                    </div>
                                    <div class="shop">
                                      <a href="#" class="shoplink">Shop Now <span><i class="fas fa-long-arrow-alt-right"></i></span></a>
                                    </div>
                                  </div>
                                  <div class="timers">
                                    <ul>
                                      <li>
                                        <p class="pcont">05</p>
                                        <span class="punit">Hours</span>
                                      </li>
                                      <li>
                                        <p class="pcont">45</p>
                                        <span class="punit">Minutes</span>
                                      </li>
                                      <li>
                                        <p class="pcont">50</p>
                                        <span class="punit">Seconds</span>
                                      </li>
                                    </ul>
                                  </div>
                                </div>
                              </div>
                              <div class="col-lg-5 col-md-5 col-sm-5 col-xs-12">
                                <div class="Daelsimg">
                                  <img src="{{asset('frontUI/assets/img/cladreximg/deals1.jpg')}}">
                                </div>
                              </div>
                            </div>
                          </div>
                        </div>

                      </div>
                    </div>
                  </div>
                </div>
                
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="display: none">
                  <div class="PopuProd">
                    <div class="title">
                      <h2 class="PopuProdtittl">New Arrival</h2>
                      <div class="line"></div>
                    </div>
                    <div class="PopuProdtemsss">
                      <div class="row">

                        <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
                          <div class="PopuProdItem">
                            <div class="imges">
                              <img src="{{asset('frontUI/assets/img/cladreximg/papa.png')}}">
                            </div>
                            <div class="details">
                              <div class="categoru">
                                <a href="#" class="catlinl">zara official</a>
                              </div>
                              <div class="nameprod">
                                <a href="#" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                              </div>
                              <div class="price">
                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                  <span class="label">FOB Price: </span>
                                  <span class="value">US $300-450 </span>
                                  <span class="unit">/ Piece</span>
                                </div>
                              </div>
                              <div class="min_order" title="Min. Order: 1 Piece">
                                <span class="label">Min. Order: </span>
                                <span class="value">1 Piece</span>
                              </div>


                              <div class="finshlinks">
                                <ul>
                                  <li>
                                    <a href="#" class="wish">
                                      <i class="icofont icofont-heart"></i>
                                    </a>
                                  </li>
                                  <li>
                                    <a href="#" class="wish">
                                      <i class="icofont icofont-chart-bar-graph"></i>
                                    </a>
                                  </li>
                                  <li class="addbuton">
                                    <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                      <i class="bx bx-basket"></i>
                                    </button>
                                  </li>
                                </ul>
                              </div>
                            </div>
                          </div>
                        </div>

                        <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
                          <div class="PopuProdItem">
                            <div class="imges">
                              <img src="{{asset('frontUI/assets/img/cladreximg/papa.png')}}">
                            </div>
                            <div class="details">
                              <div class="categoru">
                                <a href="#" class="catlinl">zara official</a>
                              </div>
                              <div class="nameprod">
                                <a href="#" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                              </div>
                              <div class="price">
                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                  <span class="label">FOB Price: </span>
                                  <span class="value">US $300-450 </span>
                                  <span class="unit">/ Piece</span>
                                </div>
                              </div>
                              <div class="min_order" title="Min. Order: 1 Piece">
                                <span class="label">Min. Order: </span>
                                <span class="value">1 Piece</span>
                              </div>


                              <div class="finshlinks">
                                <ul>
                                  <li>
                                    <a href="#" class="wish">
                                      <i class="icofont icofont-heart"></i>
                                    </a>
                                  </li>
                                  <li>
                                    <a href="#" class="wish">
                                      <i class="icofont icofont-chart-bar-graph"></i>
                                    </a>
                                  </li>
                                  <li class="addbuton">
                                    <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                      <i class="bx bx-basket"></i>
                                    </button>
                                  </li>
                                </ul>
                              </div>
                            </div>
                          </div>
                        </div>

                        <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
                          <div class="PopuProdItem">
                            <div class="imges">
                              <img src="{{asset('frontUI/assets/img/cladreximg/papa.png')}}">
                            </div>
                            <div class="details">
                              <div class="categoru">
                                <a href="#" class="catlinl">zara official</a>
                              </div>
                              <div class="nameprod">
                                <a href="#" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                              </div>
                              <div class="price">
                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                  <span class="label">FOB Price: </span>
                                  <span class="value">US $300-450 </span>
                                  <span class="unit">/ Piece</span>
                                </div>
                              </div>
                              <div class="min_order" title="Min. Order: 1 Piece">
                                <span class="label">Min. Order: </span>
                                <span class="value">1 Piece</span>
                              </div>


                              <div class="finshlinks">
                                <ul>
                                  <li>
                                    <a href="#" class="wish">
                                      <i class="icofont icofont-heart"></i>
                                    </a>
                                  </li>
                                  <li>
                                    <a href="#" class="wish">
                                      <i class="icofont icofont-chart-bar-graph"></i>
                                    </a>
                                  </li>
                                  <li class="addbuton">
                                    <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                      <i class="bx bx-basket"></i>
                                    </button>
                                  </li>
                                </ul>
                              </div>
                            </div>
                          </div>
                        </div>

                      </div>
                      <div class="seemore">
                        <a href="#" class="seemorelink">
                          see more <span><i class="fas fa-chevron-down"></i></span>
                        </a>
                      </div>
                    </div>

                  </div>
                </div>
                
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12"  style="display: none">
                  <div class="PopuProd">
                    <div class="title">
                      <h2 class="PopuProdtittl">Fashion</h2>
                      <div class="line"></div>
                    </div>
                    <div class="PopuProdtemsss">
                      <div class="row">

                        <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
                          <div class="PopuProdItem">
                            <div class="imges">
                              <img src="{{asset('frontUI/assets/img/cladreximg/papa.png')}}">
                            </div>
                            <div class="details">
                              <div class="categoru">
                                <a href="#" class="catlinl">zara official</a>
                              </div>
                              <div class="nameprod">
                                <a href="#" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                              </div>
                              <div class="price">
                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                  <span class="label">FOB Price: </span>
                                  <span class="value">US $300-450 </span>
                                  <span class="unit">/ Piece</span>
                                </div>
                              </div>
                              <div class="min_order" title="Min. Order: 1 Piece">
                                <span class="label">Min. Order: </span>
                                <span class="value">1 Piece</span>
                              </div>


                              <div class="finshlinks">
                                <ul>
                                  <li>
                                    <a href="#" class="wish">
                                      <i class="icofont icofont-heart"></i>
                                    </a>
                                  </li>
                                  <li>
                                    <a href="#" class="wish">
                                      <i class="icofont icofont-chart-bar-graph"></i>
                                    </a>
                                  </li>
                                  <li class="addbuton">
                                    <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                      <i class="bx bx-basket"></i>
                                    </button>
                                  </li>
                                </ul>
                              </div>
                            </div>
                          </div>
                        </div>

                        <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
                          <div class="PopuProdItem">
                            <div class="imges">
                              <img src="{{asset('frontUI/assets/img/cladreximg/papa.png')}}">
                            </div>
                            <div class="details">
                              <div class="categoru">
                                <a href="#" class="catlinl">zara official</a>
                              </div>
                              <div class="nameprod">
                                <a href="#" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                              </div>
                              <div class="price">
                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                  <span class="label">FOB Price: </span>
                                  <span class="value">US $300-450 </span>
                                  <span class="unit">/ Piece</span>
                                </div>
                              </div>
                              <div class="min_order" title="Min. Order: 1 Piece">
                                <span class="label">Min. Order: </span>
                                <span class="value">1 Piece</span>
                              </div>


                              <div class="finshlinks">
                                <ul>
                                  <li>
                                    <a href="#" class="wish">
                                      <i class="icofont icofont-heart"></i>
                                    </a>
                                  </li>
                                  <li>
                                    <a href="#" class="wish">
                                      <i class="icofont icofont-chart-bar-graph"></i>
                                    </a>
                                  </li>
                                  <li class="addbuton">
                                    <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                      <i class="bx bx-basket"></i>
                                    </button>
                                  </li>
                                </ul>
                              </div>
                            </div>
                          </div>
                        </div>

                        <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
                          <div class="PopuProdItem">
                            <div class="imges">
                              <img src="{{asset('frontUI/assets/img/cladreximg/papa.png')}}">
                            </div>
                            <div class="details">
                              <div class="categoru">
                                <a href="#" class="catlinl">zara official</a>
                              </div>
                              <div class="nameprod">
                                <a href="#" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                              </div>
                              <div class="price">
                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                  <span class="label">FOB Price: </span>
                                  <span class="value">US $300-450 </span>
                                  <span class="unit">/ Piece</span>
                                </div>
                              </div>
                              <div class="min_order" title="Min. Order: 1 Piece">
                                <span class="label">Min. Order: </span>
                                <span class="value">1 Piece</span>
                              </div>


                              <div class="finshlinks">
                                <ul>
                                  <li>
                                    <a href="#" class="wish">
                                      <i class="icofont icofont-heart"></i>
                                    </a>
                                  </li>
                                  <li>
                                    <a href="#" class="wish">
                                      <i class="icofont icofont-chart-bar-graph"></i>
                                    </a>
                                  </li>
                                  <li class="addbuton">
                                    <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                      <i class="bx bx-basket"></i>
                                    </button>
                                  </li>
                                </ul>
                              </div>
                            </div>
                          </div>
                        </div>

                        <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
                          <div class="PopuProdItem">
                            <div class="imges">
                              <img src="{{asset('frontUI/assets/img/cladreximg/papa.png')}}">
                            </div>
                            <div class="details">
                              <div class="categoru">
                                <a href="#" class="catlinl">zara official</a>
                              </div>
                              <div class="nameprod">
                                <a href="#" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                              </div>
                              <div class="price">
                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                  <span class="label">FOB Price: </span>
                                  <span class="value">US $300-450 </span>
                                  <span class="unit">/ Piece</span>
                                </div>
                              </div>
                              <div class="min_order" title="Min. Order: 1 Piece">
                                <span class="label">Min. Order: </span>
                                <span class="value">1 Piece</span>
                              </div>


                              <div class="finshlinks">
                                <ul>
                                  <li>
                                    <a href="#" class="wish">
                                      <i class="icofont icofont-heart"></i>
                                    </a>
                                  </li>
                                  <li>
                                    <a href="#" class="wish">
                                      <i class="icofont icofont-chart-bar-graph"></i>
                                    </a>
                                  </li>
                                  <li class="addbuton">
                                    <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                      <i class="bx bx-basket"></i>
                                    </button>
                                  </li>
                                </ul>
                              </div>
                            </div>
                          </div>
                        </div>

                        <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
                          <div class="PopuProdItem">
                            <div class="imges">
                              <img src="{{asset('frontUI/assets/img/cladreximg/papa.png')}}">
                            </div>
                            <div class="details">
                              <div class="categoru">
                                <a href="#" class="catlinl">zara official</a>
                              </div>
                              <div class="nameprod">
                                <a href="#" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                              </div>
                              <div class="price">
                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                  <span class="label">FOB Price: </span>
                                  <span class="value">US $300-450 </span>
                                  <span class="unit">/ Piece</span>
                                </div>
                              </div>
                              <div class="min_order" title="Min. Order: 1 Piece">
                                <span class="label">Min. Order: </span>
                                <span class="value">1 Piece</span>
                              </div>


                              <div class="finshlinks">
                                <ul>
                                  <li>
                                    <a href="#" class="wish">
                                      <i class="icofont icofont-heart"></i>
                                    </a>
                                  </li>
                                  <li>
                                    <a href="#" class="wish">
                                      <i class="icofont icofont-chart-bar-graph"></i>
                                    </a>
                                  </li>
                                  <li class="addbuton">
                                    <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                      <i class="bx bx-basket"></i>
                                    </button>
                                  </li>
                                </ul>
                              </div>
                            </div>
                          </div>
                        </div>

                        <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
                          <div class="PopuProdItem">
                            <div class="imges">
                              <img src="{{asset('frontUI/assets/img/cladreximg/papa.png')}}">
                            </div>
                            <div class="details">
                              <div class="categoru">
                                <a href="#" class="catlinl">zara official</a>
                              </div>
                              <div class="nameprod">
                                <a href="#" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                              </div>
                              <div class="price">
                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                  <span class="label">FOB Price: </span>
                                  <span class="value">US $300-450 </span>
                                  <span class="unit">/ Piece</span>
                                </div>
                              </div>
                              <div class="min_order" title="Min. Order: 1 Piece">
                                <span class="label">Min. Order: </span>
                                <span class="value">1 Piece</span>
                              </div>


                              <div class="finshlinks">
                                <ul>
                                  <li>
                                    <a href="#" class="wish">
                                      <i class="icofont icofont-heart"></i>
                                    </a>
                                  </li>
                                  <li>
                                    <a href="#" class="wish">
                                      <i class="icofont icofont-chart-bar-graph"></i>
                                    </a>
                                  </li>
                                  <li class="addbuton">
                                    <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                      <i class="bx bx-basket"></i>
                                    </button>
                                  </li>
                                </ul>
                              </div>
                            </div>
                          </div>
                        </div>

                      </div>
                      <div class="seemore">
                        <a href="#" class="seemorelink">
                          see more <span><i class="fas fa-chevron-down"></i></span>
                        </a>
                      </div>
                    </div>

                  </div>
                </div>



                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="display: none">
                  <div class="mediaphoto">
                    <div class="row">
                      <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                        <div class="mediaitem">
                          <a href="#" class="imgesssa">
                            <img src="{{asset('frontUI/assets/img/cladreximg/media2.png')}}">
                          </a>
                        </div>
                      </div>
                      <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                        <div class="mediaitem">
                          <a href="#" class="imgesssa">
                            <img src="{{asset('frontUI/assets/img/cladreximg/media3.png')}}">
                          </a>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                
                

                

                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="display: none">
                  <div class="PopuProd">
                    <div class="title">
                      <h2 class="PopuProdtittl">Electronics</h2>
                      <div class="line"></div>
                    </div>
                    <div class="PopuProdtemsss">
                      <div class="row">

                        <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
                          <div class="PopuProdItem">
                            <div class="imges">
                              <img src="{{asset('frontUI/assets/img/cladreximg/papa.png')}}">
                            </div>
                            <div class="details">
                              <div class="categoru">
                                <a href="#" class="catlinl">zara official</a>
                              </div>
                              <div class="nameprod">
                                <a href="#" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                              </div>
                              <div class="price">
                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                  <span class="label">FOB Price: </span>
                                  <span class="value">US $300-450 </span>
                                  <span class="unit">/ Piece</span>
                                </div>
                              </div>
                              <div class="min_order" title="Min. Order: 1 Piece">
                                <span class="label">Min. Order: </span>
                                <span class="value">1 Piece</span>
                              </div>


                              <div class="finshlinks">
                                <ul>
                                  <li>
                                    <a href="#" class="wish">
                                      <i class="icofont icofont-heart"></i>
                                    </a>
                                  </li>
                                  <li>
                                    <a href="#" class="wish">
                                      <i class="icofont icofont-chart-bar-graph"></i>
                                    </a>
                                  </li>
                                  <li class="addbuton">
                                    <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                      <i class="bx bx-basket"></i>
                                    </button>
                                  </li>
                                </ul>
                              </div>
                            </div>
                          </div>
                        </div>

                        <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
                          <div class="PopuProdItem">
                            <div class="imges">
                              <img src="{{asset('frontUI/assets/img/cladreximg/papa.png')}}">
                            </div>
                            <div class="details">
                              <div class="categoru">
                                <a href="#" class="catlinl">zara official</a>
                              </div>
                              <div class="nameprod">
                                <a href="#" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                              </div>
                              <div class="price">
                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                  <span class="label">FOB Price: </span>
                                  <span class="value">US $300-450 </span>
                                  <span class="unit">/ Piece</span>
                                </div>
                              </div>
                              <div class="min_order" title="Min. Order: 1 Piece">
                                <span class="label">Min. Order: </span>
                                <span class="value">1 Piece</span>
                              </div>


                              <div class="finshlinks">
                                <ul>
                                  <li>
                                    <a href="#" class="wish">
                                      <i class="icofont icofont-heart"></i>
                                    </a>
                                  </li>
                                  <li>
                                    <a href="#" class="wish">
                                      <i class="icofont icofont-chart-bar-graph"></i>
                                    </a>
                                  </li>
                                  <li class="addbuton">
                                    <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                      <i class="bx bx-basket"></i>
                                    </button>
                                  </li>
                                </ul>
                              </div>
                            </div>
                          </div>
                        </div>

                        <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
                          <div class="PopuProdItem">
                            <div class="imges">
                              <img src="{{asset('frontUI/assets/img/cladreximg/papa.png')}}">
                            </div>
                            <div class="details">
                              <div class="categoru">
                                <a href="#" class="catlinl">zara official</a>
                              </div>
                              <div class="nameprod">
                                <a href="#" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                              </div>
                              <div class="price">
                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                  <span class="label">FOB Price: </span>
                                  <span class="value">US $300-450 </span>
                                  <span class="unit">/ Piece</span>
                                </div>
                              </div>
                              <div class="min_order" title="Min. Order: 1 Piece">
                                <span class="label">Min. Order: </span>
                                <span class="value">1 Piece</span>
                              </div>


                              <div class="finshlinks">
                                <ul>
                                  <li>
                                    <a href="#" class="wish">
                                      <i class="icofont icofont-heart"></i>
                                    </a>
                                  </li>
                                  <li>
                                    <a href="#" class="wish">
                                      <i class="icofont icofont-chart-bar-graph"></i>
                                    </a>
                                  </li>
                                  <li class="addbuton">
                                    <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                      <i class="bx bx-basket"></i>
                                    </button>
                                  </li>
                                </ul>
                              </div>
                            </div>
                          </div>
                        </div>

                        <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
                          <div class="PopuProdItem">
                            <div class="imges">
                              <img src="{{asset('frontUI/assets/img/cladreximg/papa.png')}}">
                            </div>
                            <div class="details">
                              <div class="categoru">
                                <a href="#" class="catlinl">zara official</a>
                              </div>
                              <div class="nameprod">
                                <a href="#" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                              </div>
                              <div class="price">
                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                  <span class="label">FOB Price: </span>
                                  <span class="value">US $300-450 </span>
                                  <span class="unit">/ Piece</span>
                                </div>
                              </div>
                              <div class="min_order" title="Min. Order: 1 Piece">
                                <span class="label">Min. Order: </span>
                                <span class="value">1 Piece</span>
                              </div>


                              <div class="finshlinks">
                                <ul>
                                  <li>
                                    <a href="#" class="wish">
                                      <i class="icofont icofont-heart"></i>
                                    </a>
                                  </li>
                                  <li>
                                    <a href="#" class="wish">
                                      <i class="icofont icofont-chart-bar-graph"></i>
                                    </a>
                                  </li>
                                  <li class="addbuton">
                                    <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                      <i class="bx bx-basket"></i>
                                    </button>
                                  </li>
                                </ul>
                              </div>
                            </div>
                          </div>
                        </div>

                        <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
                          <div class="PopuProdItem">
                            <div class="imges">
                              <img src="{{asset('frontUI/assets/img/cladreximg/papa.png')}}">
                            </div>
                            <div class="details">
                              <div class="categoru">
                                <a href="#" class="catlinl">zara official</a>
                              </div>
                              <div class="nameprod">
                                <a href="#" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                              </div>
                              <div class="price">
                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                  <span class="label">FOB Price: </span>
                                  <span class="value">US $300-450 </span>
                                  <span class="unit">/ Piece</span>
                                </div>
                              </div>
                              <div class="min_order" title="Min. Order: 1 Piece">
                                <span class="label">Min. Order: </span>
                                <span class="value">1 Piece</span>
                              </div>


                              <div class="finshlinks">
                                <ul>
                                  <li>
                                    <a href="#" class="wish">
                                      <i class="icofont icofont-heart"></i>
                                    </a>
                                  </li>
                                  <li>
                                    <a href="#" class="wish">
                                      <i class="icofont icofont-chart-bar-graph"></i>
                                    </a>
                                  </li>
                                  <li class="addbuton">
                                    <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                      <i class="bx bx-basket"></i>
                                    </button>
                                  </li>
                                </ul>
                              </div>
                            </div>
                          </div>
                        </div>

                        <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
                          <div class="PopuProdItem">
                            <div class="imges">
                              <img src="{{asset('frontUI/assets/img/cladreximg/papa.png')}}">
                            </div>
                            <div class="details">
                              <div class="categoru">
                                <a href="#" class="catlinl">zara official</a>
                              </div>
                              <div class="nameprod">
                                <a href="#" class="namlink">Contec Tp500 Infrarrojo Digital Non Contact Body </a>
                              </div>
                              <div class="price">
                                <div class="prod_price" title="FOB Price: US $300-450 / Piece">
                                  <span class="label">FOB Price: </span>
                                  <span class="value">US $300-450 </span>
                                  <span class="unit">/ Piece</span>
                                </div>
                              </div>
                              <div class="min_order" title="Min. Order: 1 Piece">
                                <span class="label">Min. Order: </span>
                                <span class="value">1 Piece</span>
                              </div>


                              <div class="finshlinks">
                                <ul>
                                  <li>
                                    <a href="#" class="wish">
                                      <i class="icofont icofont-heart"></i>
                                    </a>
                                  </li>
                                  <li>
                                    <a href="#" class="wish">
                                      <i class="icofont icofont-chart-bar-graph"></i>
                                    </a>
                                  </li>
                                  <li class="addbuton">
                                    <button type="button" class="cart" data-toggle="modal" data-target="#exampleModalCenterquotaion">
                                      <i class="bx bx-basket"></i>
                                    </button>
                                  </li>
                                </ul>
                              </div>
                            </div>
                          </div>
                        </div>

                      </div>
                      <div class="seemore">
                        <a href="#" class="seemorelink">
                          see more <span><i class="fas fa-chevron-down"></i></span>
                        </a>
                      </div>
                    </div>

                  </div>
                </div>




                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="display: none">
                  <div class="tradtube">
                    <div class="title">
                      <h2 class="PopuProdtittl">Trade Tube</h2>
                      <div class="line"></div>
                    </div>
                    <div class="tardecont">
                      <div class="row">
                        <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
                          <div class="tradeitem">
                            <div class="topsec">
                              <div class="imgesss">
                                <img src="{{asset('frontUI/assets/img/cladreximg/trad1.png')}}">
                              </div>
                              <div class="overlay">
                                <a class="videoposts" href="tradetube.html">
                                  <span class="videoSpan">
                                    <i class="fas fa-play"></i>
                                  </span>
                                </a>
                              </div>
                            </div>
                            <div class="botmsec">
                              <div class="namesvedio">
                                <a href="tradetube.html" class="vediolink">مبادرة الرئيس عبد الفتاح السيسي #حياة_كريمة</a>
                              </div>
                              <div class="ownpost">
                                <div class="imgname">
                                  <img src="{{asset('frontUI/assets/img/cladreximg/own1.jpg')}}">
                                  <a class="detalown">
                                    <span class="namess">Mostafa Hamed</span>
                                    <span class="follow"> Followers</span>
                                  </a>
                                </div>
                                <div class="vewis">
                                  <ul>
                                    <li>
                                      <div class="viewitem">
                                        <a href="#" class="vicon"><i class="ti-eye fa-lg"></i></a>
                                        <span>13Views</span>
                                      </div>
                                    </li>
                                    <li>
                                      <div class="viewitem">
                                        <a href="#" class="vicon">
                                          <i class="fas fa-heart fa-lg"></i></a>
                                        <span>2</span>
                                      </div>
                                    </li>
                                  </ul>
                                </div>
                              </div>
                            </div>
                          </div>
                        </div>
                        <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
                          <div class="tradeitem">
                            <div class="topsec">
                              <div class="imgesss">
                                <img src="{{asset('frontUI/assets/img/cladreximg/trad2.png')}}">
                              </div>
                              <div class="overlay">
                                <a class="videoposts" href="tradetube.html">
                                  <span class="videoSpan">
                                    <i class="fas fa-play"></i>
                                  </span>
                                </a>
                              </div>
                            </div>
                            <div class="botmsec">
                              <div class="namesvedio">
                                <a href="tradetube.html" class="vediolink">مبادرة الرئيس عبد الفتاح السيسي #حياة_كريمة</a>
                              </div>
                              <div class="ownpost">
                                <div class="imgname">
                                  <img src="{{asset('frontUI/assets/img/cladreximg/own2.jpg')}}">
                                  <a class="detalown">
                                    <span class="namess">Mostafa Hamed</span>
                                    <span class="follow"> Followers</span>
                                  </a>
                                </div>
                                <div class="vewis">
                                  <ul>
                                    <li>
                                      <div class="viewitem">
                                        <a href="#" class="vicon"><i class="ti-eye fa-lg"></i></a>
                                        <span>13Views</span>
                                      </div>
                                    </li>
                                    <li>
                                      <div class="viewitem">
                                        <a href="#" class="vicon">
                                          <i class="fas fa-heart fa-lg"></i></a>
                                        <span>2</span>
                                      </div>
                                    </li>
                                  </ul>
                                </div>
                              </div>
                            </div>
                          </div>
                        </div>

                        <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
                          <div class="tradeitem">
                            <div class="topsec">
                              <div class="imgesss">
                                <img src="{{asset('frontUI/assets/img/cladreximg/trad2.png')}}">
                              </div>
                              <div class="overlay">
                                <a class="videoposts" href="tradetube.html">
                                  <span class="videoSpan">
                                    <i class="fas fa-play"></i>
                                  </span>
                                </a>
                              </div>
                            </div>
                            <div class="botmsec">
                              <div class="namesvedio">
                                <a href="tradetube.html" class="vediolink">مبادرة الرئيس عبد الفتاح السيسي #حياة_كريمة</a>
                              </div>
                              <div class="ownpost">
                                <div class="imgname">
                                  <img src="assets/img/cladreximg/own3.jpg">
                                  <a class="detalown">
                                    <span class="namess">Mostafa Hamed</span>
                                    <span class="follow"> Followers</span>
                                  </a>
                                </div>
                                <div class="vewis">
                                  <ul>
                                    <li>
                                      <div class="viewitem">
                                        <a href="#" class="vicon"><i class="ti-eye fa-lg"></i></a>
                                        <span>13Views</span>
                                      </div>
                                    </li>
                                    <li>
                                      <div class="viewitem">
                                        <a href="#" class="vicon">
                                          <i class="fas fa-heart fa-lg"></i></a>
                                        <span>2</span>
                                      </div>
                                    </li>
                                  </ul>
                                </div>
                              </div>
                            </div>
                          </div>
                        </div>


                      </div>
                    </div>
                  </div>
                </div>


                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="display:none">
                  <div class="store">
                    <div class="title">
                      <h2 class="PopuProdtittl">store</h2>
                      <div class="line"></div>
                    </div>
                    <div class="storeContent">
                      <div class="row">
                        <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
                          <div class="storeItem">
                            <div class="imagesss">
                              <img src="{{asset('frontUI/assets/img/cladreximg/store1.jpg')}}">
                            </div>
                            <div class="detaisl">
                              <div class="namestoe">
                                <a href="#" class="namehh">Fat Horse</a>
                              </div>
                              <div class="artilsc">
                                <p>NEWEST 2022 FASOIN</p>
                              </div>
                              <div class="liiissis">
                                <ul>
                                  <li>
                                    <p class="Followers">Products</p>
                                    <span class="unmebs"></span>
                                  </li>
                                  <li>
                                    <p class="Followers">Followers</p>
                                    <span class="unmebs">256k</span>
                                  </li>
                                  <li>
                                    <p class="Followers">Reviews</p>
                                    <span class="unmebs">4.5</span>
                                  </li>
                                </ul>
                              </div>
                              <div class="viewstore">
                                <a href="#" class="viewstorlink">View Store</a>
                              </div>
                            </div>
                          </div>
                        </div>

                        <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
                          <div class="storeItem">
                            <div class="imagesss">
                              <img src="{{asset('frontUI/assets/img/cladreximg/store2.jpg')}}">
                            </div>
                            <div class="detaisl">
                              <div class="namestoe">
                                <a href="#" class="namehh">Declan Cunningham</a>
                              </div>
                              <div class="artilsc">
                                <p>NEWEST 2022 FASOIN</p>
                              </div>
                              <div class="liiissis">
                                <ul>
                                  <li>
                                    <p class="Followers">Products</p>
                                    <span class="unmebs"></span>
                                  </li>
                                  <li>
                                    <p class="Followers">Followers</p>
                                    <span class="unmebs">256k</span>
                                  </li>
                                  <li>
                                    <p class="Followers">Reviews</p>
                                    <span class="unmebs">4.5</span>
                                  </li>
                                </ul>
                              </div>
                              <div class="viewstore">
                                <a href="#" class="viewstorlink">View Store</a>
                              </div>
                            </div>
                          </div>
                        </div>

                        <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
                          <div class="storeItem">
                            <div class="imagesss">
                              <img src="{{asset('frontUI/assets/img/cladreximg/store3.jpg')}}">
                            </div>
                            <div class="detaisl">
                              <div class="namestoe">
                                <a href="#" class="namehh">Shea Johnston</a>
                              </div>
                              <div class="artilsc">
                                <p>NEWEST 2022 FASOIN</p>
                              </div>
                              <div class="liiissis">
                                <ul>
                                  <li>
                                    <p class="Followers">Products</p>
                                    <span class="unmebs"></span>
                                  </li>
                                  <li>
                                    <p class="Followers">Followers</p>
                                    <span class="unmebs">256k</span>
                                  </li>
                                  <li>
                                    <p class="Followers">Reviews</p>
                                    <span class="unmebs">4.5</span>
                                  </li>
                                </ul>
                              </div>
                              <div class="viewstore">
                                <a href="#" class="viewstorlink">View Store</a>
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                
                
                <div class="col-lg-12" >
                                   
                                      <div class="PrpServApp">
                                    <div class="row">
                                        <div class="col-lg-3 col-md-6 col-sm-6 col-xs-12">
                                            <div class="PropServApp_Item">
                                                <div class="icon">
                                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 511.99928 511" src="assets/images/icons/service_1.svg" class="svg_img" alt=""><path d="m241.21875 257.140625c-3.910156 3.898437-3.910156 10.230469 0 14.140625s10.238281 3.910156 14.140625 0c3.910156-3.910156 3.910156-10.242188 0-14.140625-3.902344-3.910156-10.230469-3.910156-14.140625 0zm0 0"></path><path d="m400.226562 29.753906c-17.160156 13.285156-21.835937 20.132813-82.695312 80.363282l-36.484375-5.941407 16.742187-16.742187c11.726563-11.726563 11.730469-30.699219 0-42.425782-11.722656-11.722656-30.707031-11.726562-42.429687 0l-47.289063 47.289063-165.382812-26.925781c-10.6875-1.734375-21.621094 1.800781-29.269531 9.449218-14.707031 14.707032-12.710938 39.039063 4.066406 51.179688l164.808594 119.351562-30.855469 30.859376c-10.574219 10.574218-21.703125 25.117187-33.140625 43.277343l-76.859375-22.601562c-11.34375-3.332031-23.59375-.210938-31.972656 8.152343-14.265625 14.265626-12.109375 37.957032 4.460937 49.433594l63.007813 43.617188c-10.96875 24.207031-17.636719 41.710937-17.945313 42.519531-1.398437 3.679687-.503906 7.839844 2.277344 10.621094 2.796875 2.796875 6.960937 3.667969 10.621094 2.277343.808593-.304687 18.3125-6.976562 42.519531-17.945312l43.621094 63.011719c11.496094 16.597656 35.1875 18.699219 49.4375 4.449219 8.359375-8.371094 11.480468-20.621094 8.144531-31.964844l-22.597656-76.855469c18.171875-11.445313 32.707031-22.574219 43.277343-33.140625l30.859376-30.859375 119.347656 164.808594c12.160156 16.804687 36.492187 18.757812 51.183594 4.066406 7.644531-7.644531 11.179687-18.585937 9.449218-29.277344l-26.925781-165.371093 47.285156-47.285157c11.726563-11.726562 11.726563-30.710937 0-42.433593-11.722656-11.726563-30.695312-11.726563-42.421875 0l-16.746094 16.742187-5.9375-36.484375c60.292969-60.921875 67.03125-65.488281 80.324219-82.648438.015625-.019531.035157-.042968.050781-.066406 30.84375-39.972656 38.0625-80.34375 17.949219-100.464844-20.117187-20.105468-60.492187-12.894531-100.480469 17.964844zm-130.726562 29.398438c3.90625-3.910156 10.238281-3.910156 14.148438 0 3.910156 3.910156 3.90625 10.226562 0 14.136718l-26.929688 26.925782-24.324219-3.960938zm-240.289062 50.644531c-6.84375-4.949219-7.628907-14.859375-1.648438-20.835937 3.113281-3.117188 7.574219-4.558594 11.917969-3.851563l260.625 42.433594-103.488281 103.484375zm-5.605469 209.390625c3.195312-3.191406 7.867187-4.386719 12.191406-3.113281l71.886719 21.140625c-7.351563 12.875-14.769532 27.097656-22.167969 42.492187l-60.203125-41.675781c-6.316406-4.375-7.144531-13.410156-1.707031-18.84375zm169.710937 169.703125c-5.390625 5.390625-14.4375 4.664063-18.847656-1.703125l-41.675781-60.207031c15.40625-7.40625 29.632812-14.828125 42.492187-22.164063l21.136719 71.882813c1.273437 4.328125.082031 9-3.105469 12.191406zm245.894532-260.039063c3.90625-3.90625 10.226562-3.910156 14.136718 0 3.910156 3.910157 3.90625 10.238282 0 14.148438l-37.105468 37.101562-3.957032-24.324218zm-11.824219 244.15625c.707031 4.355469-.734375 8.816407-3.847657 11.929688-5.949218 5.949219-15.867187 5.222656-20.839843-1.652344l-121.230469-167.402344 103.488281-103.488281zm-205.238281-126.089843c-11.003907 11.007812-27.109376 22.890625-47.867188 35.324219-26.984375 16.160156-58.917969 31.464843-87.722656 43.699218 12.566406-29.585937 28.160156-62.035156 44.667968-89.328125 12.074219-19.976562 23.632813-35.542969 34.355469-46.261719 244.273438-243.6875 229.800781-231.117187 244.625-242.972656 16.445313-1.050781 30.921875 3.832032 41 13.910156 10.078125 10.078126 14.964844 24.558594 13.914063 41-11.882813 14.859376.539062.527344-242.972656 244.628907zm260.085937-270.90625c-3.195313-11.023438-8.871094-20.851563-16.882813-28.863281-8.011718-8.007813-17.839843-13.6875-28.859374-16.882813 14.472656-7.339844 38.84375-15.558594 50.074218-4.335937 8.867188 8.871093 6.664063 28.398437-4.332031 50.082031zm0 0"></path><path d="m297.789062 200.570312-28.28125 28.277344c-3.90625 3.90625-3.90625 10.238282 0 14.144532s10.234376 3.902343 14.144532 0l28.277344-28.28125c3.90625-3.902344 3.90625-10.234376 0-14.140626s-10.234376-3.90625-14.140626 0zm0 0"></path><path d="m354.359375 144-28.292969 28.289062c-3.902344 3.90625-3.902344 10.238282 0 14.144532 3.910156 3.902344 10.238282 3.902344 14.144532 0l28.289062-28.289063c3.90625-3.90625 3.90625-10.238281 0-14.144531s-10.234375-3.90625-14.140625 0zm0 0"></path></svg>
                                                </div>
                                                <div class="Detaila">
                                                    <!--<h2>Free Shipping</h2>-->
                                                    <p> {{__('front.Free shipping on order above $200')}} </p>
                                                </div>
                                            </div>
                                        </div>
                                        
                                        <div class="col-lg-3 col-md-6 col-sm-6 col-xs-12">
                                            <div class="PropServApp_Item">
                                                <div class="icon">
                                                    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" id="Capa_1" x="0px" y="0px" viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xml:space="preserve" src="assets/images/icons/service_2.svg" class="svg_img" alt="">
<g>
	<g>
		<g>
			<path d="M477.679,222.71C473.752,99.221,375.854,0,256,0S38.248,99.221,34.321,222.71C14.387,226.709,0.032,244.202,0,264.533     V384c0.028,23.553,19.114,42.639,42.667,42.667c0.049,42.395,34.405,76.751,76.8,76.8h79.173     c3.024,5.262,8.624,8.514,14.693,8.533h76.8c9.422-0.009,17.057-7.645,17.067-17.067V460.8     c-0.009-9.422-7.645-17.057-17.067-17.067h-76.8c-6.07,0.019-11.669,3.271-14.693,8.533h-79.173     c-14.132-0.015-25.585-11.468-25.6-25.6h25.6c9.422-0.009,17.057-7.645,17.067-17.067V238.933     c-0.009-9.422-7.645-17.057-17.067-17.067H85.745C90.03,127.028,164.639,51.2,256,51.2s165.97,75.828,170.255,170.667h-33.721     c-9.422,0.009-17.057,7.645-17.067,17.067V409.6c0.009,9.422,7.645,17.057,17.067,17.067h76.8     C492.886,426.639,511.972,407.553,512,384V264.533C511.968,244.202,497.613,226.709,477.679,222.71z M213.333,460.8h76.8     l0.012,34.133h-76.812V460.8z M119.467,469.333h76.8V486.4h-76.8c-32.974-0.037-59.696-26.759-59.733-59.733H76.8     C76.828,450.219,95.914,469.306,119.467,469.333z M119.467,238.933l0.012,170.667H42.667c-14.132-0.015-25.585-11.468-25.6-25.6     V264.533c0.015-14.132,11.468-25.585,25.6-25.6H119.467z M256,34.133c-100.779,0-183.234,83.459-187.526,187.733H51.408     c4.32-113.687,94.415-204.8,204.592-204.8s200.272,91.113,204.592,204.8h-17.066C439.234,117.593,356.779,34.133,256,34.133z      M494.933,384c-0.015,14.132-11.468,25.585-25.6,25.6h-76.8V238.933h76.8c14.132,0.015,25.585,11.468,25.6,25.6V384z"></path>
			<path d="M349.867,332.8H332.8v-21.333c0-4.713-3.82-8.533-8.533-8.533s-8.533,3.82-8.533,8.533V332.8h-36.162l43.446-72.408     c1.569-2.614,1.624-5.867,0.145-8.532c-1.479-2.666-4.268-4.34-7.317-4.392c-3.048-0.052-5.893,1.527-7.462,4.141l-51.2,85.333     c-1.582,2.636-1.623,5.919-0.109,8.595s4.351,4.33,7.425,4.33h51.233v25.6c0,4.713,3.82,8.533,8.533,8.533     s8.533-3.82,8.533-8.533v-25.6h17.067c4.713,0,8.533-3.82,8.533-8.533S354.58,332.8,349.867,332.8z"></path>
			<path d="M158.313,367.838c-3.535,1.773-5.389,5.743-4.48,9.592s4.345,6.568,8.3,6.57h76.8c4.713,0,8.533-3.82,8.533-8.533     s-3.82-8.533-8.533-8.533H191.45c21.067-15.825,47.483-42.5,47.483-76.8c0-17.745-10.983-33.637-27.582-39.911     c-16.599-6.274-35.348-1.619-47.085,11.69c-3.118,3.535-2.78,8.928,0.754,12.046c3.535,3.118,8.928,2.78,12.046-0.754     c7.041-7.987,18.291-10.782,28.251-7.018c9.96,3.764,16.55,13.3,16.549,23.947C221.867,335.4,158.946,367.517,158.313,367.838z"></path>
		</g>
	</g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
</svg>
                                                </div>
                                                <div class="Detaila">
                                                    <!--<h2>24X7 Support</h2>-->
                                                    <p>{{__('front.Contact us 24 hours a day, 7 days a week')}} </p>
                                                </div>
                                            </div>
                                        </div>
                                        
                                        <div class="col-lg-3 col-md-6 col-sm-6 col-xs-12">
                                            <div class="PropServApp_Item">
                                                <div class="icon">
                                                    <svg xmlns="http://www.w3.org/2000/svg" height="511pt" viewBox="0 1 511.99981 511" width="511pt" src="assets/images/icons/service_3.svg" class="svg_img" alt=""><path d="m506.8125 111.230469-199.40625-109.496094c-2.996094-1.6445312-6.628906-1.6445312-9.625 0l-75.160156 41.269531c-1.585938.417969-3.066406 1.214844-4.292969 2.359375l-119.949219 65.867188c-3.199218 1.753906-5.1875 5.113281-5.1875 8.765625v132.429687c-20.234375 6.328125-38.777344 17.488281-54.195312 32.910157-40.1875 40.183593-50.433594 101.472656-25.496094 152.507812 2.421875 4.960938 8.410156 7.019531 13.371094 4.59375 4.964844-2.425781 7.019531-8.414062 4.597656-13.375-21.1875-43.363281-12.480469-95.4375 21.667969-129.585938 21.355469-21.355468 49.746093-33.117187 79.945312-33.117187s58.589844 11.761719 79.945313 33.117187c21.351562 21.351563 33.113281 49.742188 33.113281 79.941407s-11.761719 58.589843-33.117187 79.945312c-34.148438 34.148438-86.222657 42.855469-129.585938 21.667969-4.960938-2.425781-10.949219-.367188-13.371094 4.59375-2.425781 4.964844-.371094 10.953125 4.59375 13.375 18.589844 9.085938 38.535156 13.5 58.335938 13.5 34.542968-.003906 68.625-13.449219 94.171875-38.996094 11.714843-11.714844 20.972656-25.230468 27.523437-39.921875l43.089844 23.660157c1.5.824218 3.15625 1.234374 4.8125 1.234374s3.316406-.410156 4.8125-1.234374l199.40625-109.492188c3.199219-1.757812 5.1875-5.117188 5.1875-8.765625v-69.496094c0-5.523437-4.476562-10-10-10s-10 4.476563-10 10v63.578125l-179.375 98.496094v-196.171875l59.199219-32.507813v51.53125c0 3.539063 1.867187 6.8125 4.910156 8.609376 1.570313.925781 3.328125 1.390624 5.089844 1.390624 1.65625 0 3.316406-.410156 4.820312-1.238281l42.730469-23.519531c3.195312-1.757812 5.175781-5.113281 5.175781-8.757812v-62.460938l57.449219-31.542969v52.597657c0 5.523437 4.476562 10 10 10s10-4.476563 10-10v-69.496094c0-3.648438-1.988281-7.011719-5.1875-8.765625zm-204.21875-89.324219 178.628906 98.089844-56.347656 30.941406-178.628906-98.089844zm0 196.175781-178.628906-98.085937 58.414062-32.078125 178.632813 98.085937zm79.191406-43.484375-178.628906-98.085937 22.3125-12.253907 178.632812 98.085938zm-154.617187 110.738282c-25.132813-25.132813-58.546875-38.972657-94.085938-38.972657-6.722656 0-13.363281.496094-19.890625 1.46875v-110.9375l179.4375 98.53125v196.171875l-31.144531-17.101562c3.066406-11.289063 4.652344-23.0625 4.652344-35.078125 0-35.539063-13.839844-68.953125-38.96875-94.082031zm187.386719-60.347657-22.730469 12.511719v-45.597656l22.730469-12.480469zm0 0"></path><path d="m502 219.441406c-2.628906 0-5.210938 1.070313-7.070312 2.929688-1.859376 1.859375-2.929688 4.4375-2.929688 7.070312 0 2.628906 1.070312 5.207032 2.929688 7.066406 1.859374 1.863282 4.441406 2.929688 7.070312 2.929688s5.210938-1.066406 7.070312-2.929688c1.859376-1.859374 2.929688-4.4375 2.929688-7.066406 0-2.632812-1.070312-5.210937-2.929688-7.070312-1.859374-1.859375-4.441406-2.929688-7.070312-2.929688zm0 0"></path><path d="m99.457031 389.417969c2.558594 0 5.121094-.976563 7.070313-2.925781 3.90625-3.90625 3.90625-10.238282 0-14.144532l-6.925782-6.929687h59.101563c14.335937 0 26 11.664062 26 26 0 14.335937-11.664063 26-26 26h-35.019531c-5.523438 0-10 4.476562-10 10 0 5.519531 4.476562 9.996093 10 9.996093h35.019531c25.363281 0 46-20.632812 46-45.996093s-20.636719-45.996094-46-45.996094h-59.101563l6.925782-6.929687c3.90625-3.90625 3.90625-10.238282 0-14.144532-3.902344-3.902344-10.234375-3.902344-14.140625 0l-24 24c-3.902344 3.90625-3.902344 10.238282 0 14.144532l24 23.996093c1.953125 1.953125 4.511719 2.929688 7.070312 2.929688zm0 0"></path><path d="m46.074219 476.449219c-2.84375 0-5.667969-1.210938-7.640625-3.558594l-.019532-.023437c-3.554687-4.226563-3.007812-10.53125 1.21875-14.085938 4.226563-3.558594 10.535157-3.011719 14.089844 1.214844 3.550782 4.230468 3.015625 10.546875-1.210937 14.101562-1.878907 1.578125-4.164063 2.351563-6.4375 2.351563zm0 0"></path></svg>
                                                </div>
                                                <div class="Detaila">
                                                    <!--<h2>30 Days Return</h2>-->
                                                    <p>{{__('front.Simply return it within 30 days for an exchange')}} </p>
                                                </div>
                                            </div>
                                        </div>
                                        
                                        <div class="col-lg-3 col-md-6 col-sm-6 col-xs-12">
                                            <div class="PropServApp_Item">
                                                <div class="icon">
                                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" width="512" height="512" src="assets/images/icons/service_4.svg" class="svg_img" alt=""><g id="_04-security" data-name="04-security"><g id="linear_color-2" data-name="linear color"><path d="M160,207H48a10,10,0,0,0-10,10v32a10,10,0,0,0,10,10H160a10,10,0,0,0,10-10V217A10,10,0,0,0,160,207Zm-10,32H58V227h92Z"></path><path d="M112,278H49a10,10,0,0,0,0,20h63a10,10,0,0,0,0-20Z"></path><path d="M506.347,328.917c-1.114-29.639-3.358-57.449-3.38-57.727a10,10,0,0,0-7.151-8.785L442,246.605V80a26.029,26.029,0,0,0-26-26H32A26.029,26.029,0,0,0,6,80V336a26.029,26.029,0,0,0,26,26H261.022c.157,28.336,2.652,37.709,5.034,42.472,1.785,3.57,6.526,13.053,60.4,57.92,25.953,21.615,51.141,41.294,51.393,41.49a10,10,0,0,0,12.306,0c.252-.2,25.44-19.875,51.393-41.49,53.872-44.867,58.613-54.35,60.4-57.92C505.08,398.2,508.414,383.945,506.347,328.917ZM26,138H422v28H26Zm6-64H416a6.006,6.006,0,0,1,6,6v38H26V80A6.006,6.006,0,0,1,32,74ZM26,336V186H422v54.734l-35.184-10.329a9.992,9.992,0,0,0-5.632,0l-109,32a10,10,0,0,0-7.151,8.785c-.022.278-2.266,28.088-3.38,57.727-.174,4.647-.309,9-.41,13.083H32A6.006,6.006,0,0,1,26,336Zm458.056,59.528C479.3,405.038,430.55,446.661,384,483.294c-46.549-36.631-95.3-78.253-100.056-87.766-.625-1.25-3.279-8.655-2.881-43.382,0-.049.007-.1.007-.146,0-.031,0-.061,0-.091.076-6.421.255-13.768.571-22.179.768-20.486,2.086-40.1,2.812-50.082L384,250.422l99.551,29.226c.725,9.969,2.041,29.544,2.81,50.019C488.4,383.89,484.8,394.043,484.056,395.528Z"></path><path d="M457.851,292.947l-71.034-20.854a10,10,0,0,0-5.634,0l-71.034,20.854a10,10,0,0,0-7.165,9.006c-1.709,28.945-2.9,65.572-1.106,81.565a9.992,9.992,0,0,0,2.621,5.7c12.194,13.09,45.82,41.377,73.222,63.484a10,10,0,0,0,12.558,0c27.4-22.107,61.028-50.394,73.222-63.484a9.992,9.992,0,0,0,2.621-5.7c1.79-15.993.6-52.621-1.106-81.565A10,10,0,0,0,457.851,292.947ZM446.54,378c-10.431,10.483-34,30.861-62.54,54.058-28.535-23.2-52.109-43.575-62.54-54.058-.908-12.826-.5-38.308,1.093-67.848L384,292.11l61.447,18.04C447.04,339.689,447.448,365.171,446.54,378Z"></path><path d="M422.247,320.191a10,10,0,0,0-14.056,1.562l-26.272,32.841L365.547,343.68a10,10,0,0,0-11.094,16.64l24,16a10,10,0,0,0,13.356-2.073l32-40A10,10,0,0,0,422.247,320.191Z"></path></g></g></svg>
                                                </div>
                                                <div class="Detaila">
                                                    <!--<h2>Payment Secure</h2>-->
                                                    <p>{{__('front.Contact us 24 hours a day, 7 days a week')}} </p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                
                               </div>
                               
                <div class="col-lg-12 col-md-12">
                                    <div class="footerNew">
                                        <p> {{__('front.Copyright')}} </p>
                                    </div>
                                </div>
                  
                
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="display: none">
                  <div class="footercenter">
                    <div class="footertop">
                      <div class="row">
                        <!--<div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">-->

                        <!--  <div class="footitem">-->
                        <!--    <div class="icon">-->
                        <!--      <a href="#"><i class=" fas fa-money-check-alt"></i></a>-->
                        <!--    </div>-->
                        <!--    <div class="detaisl">-->
                        <!--      <h2>Greate Value</h2>-->
                        <!--      <p>We Offer Competitive Prices on our 100 Million + Products</p>-->
                        <!--    </div>-->
                        <!--  </div>-->

                        <!--</div>-->
                        <!--<div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">-->

                        <!--  <div class="footitem">-->
                        <!--    <div class="icon">-->
                        <!--      <a href="#"><i class="fa fa-motorcycle"></i></a>-->
                        <!--    </div>-->
                        <!--    <div class="detaisl">-->
                        <!--      <h2>WorldWide Delivery</h2>-->
                        <!--      <p>With Sites In Many Languages, We Ship to Over 100 Countries & Regions</p>-->
                        <!--    </div>-->
                        <!--  </div>-->

                        <!--</div>-->
                        <!--<div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">-->

                        <!--  <div class="footitem">-->
                        <!--    <div class="icon">-->
                        <!--      <a href="#"><i class="fa fa-credit-card"></i></a>-->
                        <!--    </div>-->
                        <!--    <div class="detaisl">-->
                        <!--      <h2>Safe Payment</h2>-->
                        <!--      <p>Pay With The World`s Most Popular And Secure Payment Methods</p>-->
                        <!--    </div>-->
                        <!--  </div>-->

                        <!--</div>-->
                        <!--<div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">-->

                        <!--  <div class="footitem">-->
                        <!--    <div class="icon">-->
                        <!--      <a href="#"><i class=" fas fa-check-double"></i></a>-->
                        <!--    </div>-->
                        <!--    <div class="detaisl">-->
                        <!--      <h2>Shop With Confidence</h2>-->
                        <!--      <p>Our Buyer Protection Covers Your Purchase From Clicks to Delivery</p>-->
                        <!--    </div>-->
                        <!--  </div>-->

                        <!--</div>-->
                        
                        <div class="col-lg-12">
                                   
                                      <div class="PrpServApp">
                                    <div class="row">
                                        <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
                                            <div class="PropServApp_Item">
                                                <div class="icon">
                                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 511.99928 511" src="assets/images/icons/service_1.svg" class="svg_img" alt=""><path d="m241.21875 257.140625c-3.910156 3.898437-3.910156 10.230469 0 14.140625s10.238281 3.910156 14.140625 0c3.910156-3.910156 3.910156-10.242188 0-14.140625-3.902344-3.910156-10.230469-3.910156-14.140625 0zm0 0"></path><path d="m400.226562 29.753906c-17.160156 13.285156-21.835937 20.132813-82.695312 80.363282l-36.484375-5.941407 16.742187-16.742187c11.726563-11.726563 11.730469-30.699219 0-42.425782-11.722656-11.722656-30.707031-11.726562-42.429687 0l-47.289063 47.289063-165.382812-26.925781c-10.6875-1.734375-21.621094 1.800781-29.269531 9.449218-14.707031 14.707032-12.710938 39.039063 4.066406 51.179688l164.808594 119.351562-30.855469 30.859376c-10.574219 10.574218-21.703125 25.117187-33.140625 43.277343l-76.859375-22.601562c-11.34375-3.332031-23.59375-.210938-31.972656 8.152343-14.265625 14.265626-12.109375 37.957032 4.460937 49.433594l63.007813 43.617188c-10.96875 24.207031-17.636719 41.710937-17.945313 42.519531-1.398437 3.679687-.503906 7.839844 2.277344 10.621094 2.796875 2.796875 6.960937 3.667969 10.621094 2.277343.808593-.304687 18.3125-6.976562 42.519531-17.945312l43.621094 63.011719c11.496094 16.597656 35.1875 18.699219 49.4375 4.449219 8.359375-8.371094 11.480468-20.621094 8.144531-31.964844l-22.597656-76.855469c18.171875-11.445313 32.707031-22.574219 43.277343-33.140625l30.859376-30.859375 119.347656 164.808594c12.160156 16.804687 36.492187 18.757812 51.183594 4.066406 7.644531-7.644531 11.179687-18.585937 9.449218-29.277344l-26.925781-165.371093 47.285156-47.285157c11.726563-11.726562 11.726563-30.710937 0-42.433593-11.722656-11.726563-30.695312-11.726563-42.421875 0l-16.746094 16.742187-5.9375-36.484375c60.292969-60.921875 67.03125-65.488281 80.324219-82.648438.015625-.019531.035157-.042968.050781-.066406 30.84375-39.972656 38.0625-80.34375 17.949219-100.464844-20.117187-20.105468-60.492187-12.894531-100.480469 17.964844zm-130.726562 29.398438c3.90625-3.910156 10.238281-3.910156 14.148438 0 3.910156 3.910156 3.90625 10.226562 0 14.136718l-26.929688 26.925782-24.324219-3.960938zm-240.289062 50.644531c-6.84375-4.949219-7.628907-14.859375-1.648438-20.835937 3.113281-3.117188 7.574219-4.558594 11.917969-3.851563l260.625 42.433594-103.488281 103.484375zm-5.605469 209.390625c3.195312-3.191406 7.867187-4.386719 12.191406-3.113281l71.886719 21.140625c-7.351563 12.875-14.769532 27.097656-22.167969 42.492187l-60.203125-41.675781c-6.316406-4.375-7.144531-13.410156-1.707031-18.84375zm169.710937 169.703125c-5.390625 5.390625-14.4375 4.664063-18.847656-1.703125l-41.675781-60.207031c15.40625-7.40625 29.632812-14.828125 42.492187-22.164063l21.136719 71.882813c1.273437 4.328125.082031 9-3.105469 12.191406zm245.894532-260.039063c3.90625-3.90625 10.226562-3.910156 14.136718 0 3.910156 3.910157 3.90625 10.238282 0 14.148438l-37.105468 37.101562-3.957032-24.324218zm-11.824219 244.15625c.707031 4.355469-.734375 8.816407-3.847657 11.929688-5.949218 5.949219-15.867187 5.222656-20.839843-1.652344l-121.230469-167.402344 103.488281-103.488281zm-205.238281-126.089843c-11.003907 11.007812-27.109376 22.890625-47.867188 35.324219-26.984375 16.160156-58.917969 31.464843-87.722656 43.699218 12.566406-29.585937 28.160156-62.035156 44.667968-89.328125 12.074219-19.976562 23.632813-35.542969 34.355469-46.261719 244.273438-243.6875 229.800781-231.117187 244.625-242.972656 16.445313-1.050781 30.921875 3.832032 41 13.910156 10.078125 10.078126 14.964844 24.558594 13.914063 41-11.882813 14.859376.539062.527344-242.972656 244.628907zm260.085937-270.90625c-3.195313-11.023438-8.871094-20.851563-16.882813-28.863281-8.011718-8.007813-17.839843-13.6875-28.859374-16.882813 14.472656-7.339844 38.84375-15.558594 50.074218-4.335937 8.867188 8.871093 6.664063 28.398437-4.332031 50.082031zm0 0"></path><path d="m297.789062 200.570312-28.28125 28.277344c-3.90625 3.90625-3.90625 10.238282 0 14.144532s10.234376 3.902343 14.144532 0l28.277344-28.28125c3.90625-3.902344 3.90625-10.234376 0-14.140626s-10.234376-3.90625-14.140626 0zm0 0"></path><path d="m354.359375 144-28.292969 28.289062c-3.902344 3.90625-3.902344 10.238282 0 14.144532 3.910156 3.902344 10.238282 3.902344 14.144532 0l28.289062-28.289063c3.90625-3.90625 3.90625-10.238281 0-14.144531s-10.234375-3.90625-14.140625 0zm0 0"></path></svg>
                                                </div>
                                                <div class="Detaila">
                                                    <h2>Free Shipping</h2>
                                                    <p>Free shipping on all US order or order above $200</p>
                                                </div>
                                            </div>
                                        </div>
                                        
                                        <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
                                            <div class="PropServApp_Item">
                                                <div class="icon">
                                                    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" id="Capa_1" x="0px" y="0px" viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xml:space="preserve" src="assets/images/icons/service_2.svg" class="svg_img" alt="">
<g>
	<g>
		<g>
			<path d="M477.679,222.71C473.752,99.221,375.854,0,256,0S38.248,99.221,34.321,222.71C14.387,226.709,0.032,244.202,0,264.533     V384c0.028,23.553,19.114,42.639,42.667,42.667c0.049,42.395,34.405,76.751,76.8,76.8h79.173     c3.024,5.262,8.624,8.514,14.693,8.533h76.8c9.422-0.009,17.057-7.645,17.067-17.067V460.8     c-0.009-9.422-7.645-17.057-17.067-17.067h-76.8c-6.07,0.019-11.669,3.271-14.693,8.533h-79.173     c-14.132-0.015-25.585-11.468-25.6-25.6h25.6c9.422-0.009,17.057-7.645,17.067-17.067V238.933     c-0.009-9.422-7.645-17.057-17.067-17.067H85.745C90.03,127.028,164.639,51.2,256,51.2s165.97,75.828,170.255,170.667h-33.721     c-9.422,0.009-17.057,7.645-17.067,17.067V409.6c0.009,9.422,7.645,17.057,17.067,17.067h76.8     C492.886,426.639,511.972,407.553,512,384V264.533C511.968,244.202,497.613,226.709,477.679,222.71z M213.333,460.8h76.8     l0.012,34.133h-76.812V460.8z M119.467,469.333h76.8V486.4h-76.8c-32.974-0.037-59.696-26.759-59.733-59.733H76.8     C76.828,450.219,95.914,469.306,119.467,469.333z M119.467,238.933l0.012,170.667H42.667c-14.132-0.015-25.585-11.468-25.6-25.6     V264.533c0.015-14.132,11.468-25.585,25.6-25.6H119.467z M256,34.133c-100.779,0-183.234,83.459-187.526,187.733H51.408     c4.32-113.687,94.415-204.8,204.592-204.8s200.272,91.113,204.592,204.8h-17.066C439.234,117.593,356.779,34.133,256,34.133z      M494.933,384c-0.015,14.132-11.468,25.585-25.6,25.6h-76.8V238.933h76.8c14.132,0.015,25.585,11.468,25.6,25.6V384z"></path>
			<path d="M349.867,332.8H332.8v-21.333c0-4.713-3.82-8.533-8.533-8.533s-8.533,3.82-8.533,8.533V332.8h-36.162l43.446-72.408     c1.569-2.614,1.624-5.867,0.145-8.532c-1.479-2.666-4.268-4.34-7.317-4.392c-3.048-0.052-5.893,1.527-7.462,4.141l-51.2,85.333     c-1.582,2.636-1.623,5.919-0.109,8.595s4.351,4.33,7.425,4.33h51.233v25.6c0,4.713,3.82,8.533,8.533,8.533     s8.533-3.82,8.533-8.533v-25.6h17.067c4.713,0,8.533-3.82,8.533-8.533S354.58,332.8,349.867,332.8z"></path>
			<path d="M158.313,367.838c-3.535,1.773-5.389,5.743-4.48,9.592s4.345,6.568,8.3,6.57h76.8c4.713,0,8.533-3.82,8.533-8.533     s-3.82-8.533-8.533-8.533H191.45c21.067-15.825,47.483-42.5,47.483-76.8c0-17.745-10.983-33.637-27.582-39.911     c-16.599-6.274-35.348-1.619-47.085,11.69c-3.118,3.535-2.78,8.928,0.754,12.046c3.535,3.118,8.928,2.78,12.046-0.754     c7.041-7.987,18.291-10.782,28.251-7.018c9.96,3.764,16.55,13.3,16.549,23.947C221.867,335.4,158.946,367.517,158.313,367.838z"></path>
		</g>
	</g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
</svg>
                                                </div>
                                                <div class="Detaila">
                                                    <h2>24X7 Support</h2>
                                                    <p>Contact us 24 hours a day, 7 days a week</p>
                                                </div>
                                            </div>
                                        </div>
                                        
                                        <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
                                            <div class="PropServApp_Item">
                                                <div class="icon">
                                                    <svg xmlns="http://www.w3.org/2000/svg" height="511pt" viewBox="0 1 511.99981 511" width="511pt" src="assets/images/icons/service_3.svg" class="svg_img" alt=""><path d="m506.8125 111.230469-199.40625-109.496094c-2.996094-1.6445312-6.628906-1.6445312-9.625 0l-75.160156 41.269531c-1.585938.417969-3.066406 1.214844-4.292969 2.359375l-119.949219 65.867188c-3.199218 1.753906-5.1875 5.113281-5.1875 8.765625v132.429687c-20.234375 6.328125-38.777344 17.488281-54.195312 32.910157-40.1875 40.183593-50.433594 101.472656-25.496094 152.507812 2.421875 4.960938 8.410156 7.019531 13.371094 4.59375 4.964844-2.425781 7.019531-8.414062 4.597656-13.375-21.1875-43.363281-12.480469-95.4375 21.667969-129.585938 21.355469-21.355468 49.746093-33.117187 79.945312-33.117187s58.589844 11.761719 79.945313 33.117187c21.351562 21.351563 33.113281 49.742188 33.113281 79.941407s-11.761719 58.589843-33.117187 79.945312c-34.148438 34.148438-86.222657 42.855469-129.585938 21.667969-4.960938-2.425781-10.949219-.367188-13.371094 4.59375-2.425781 4.964844-.371094 10.953125 4.59375 13.375 18.589844 9.085938 38.535156 13.5 58.335938 13.5 34.542968-.003906 68.625-13.449219 94.171875-38.996094 11.714843-11.714844 20.972656-25.230468 27.523437-39.921875l43.089844 23.660157c1.5.824218 3.15625 1.234374 4.8125 1.234374s3.316406-.410156 4.8125-1.234374l199.40625-109.492188c3.199219-1.757812 5.1875-5.117188 5.1875-8.765625v-69.496094c0-5.523437-4.476562-10-10-10s-10 4.476563-10 10v63.578125l-179.375 98.496094v-196.171875l59.199219-32.507813v51.53125c0 3.539063 1.867187 6.8125 4.910156 8.609376 1.570313.925781 3.328125 1.390624 5.089844 1.390624 1.65625 0 3.316406-.410156 4.820312-1.238281l42.730469-23.519531c3.195312-1.757812 5.175781-5.113281 5.175781-8.757812v-62.460938l57.449219-31.542969v52.597657c0 5.523437 4.476562 10 10 10s10-4.476563 10-10v-69.496094c0-3.648438-1.988281-7.011719-5.1875-8.765625zm-204.21875-89.324219 178.628906 98.089844-56.347656 30.941406-178.628906-98.089844zm0 196.175781-178.628906-98.085937 58.414062-32.078125 178.632813 98.085937zm79.191406-43.484375-178.628906-98.085937 22.3125-12.253907 178.632812 98.085938zm-154.617187 110.738282c-25.132813-25.132813-58.546875-38.972657-94.085938-38.972657-6.722656 0-13.363281.496094-19.890625 1.46875v-110.9375l179.4375 98.53125v196.171875l-31.144531-17.101562c3.066406-11.289063 4.652344-23.0625 4.652344-35.078125 0-35.539063-13.839844-68.953125-38.96875-94.082031zm187.386719-60.347657-22.730469 12.511719v-45.597656l22.730469-12.480469zm0 0"></path><path d="m502 219.441406c-2.628906 0-5.210938 1.070313-7.070312 2.929688-1.859376 1.859375-2.929688 4.4375-2.929688 7.070312 0 2.628906 1.070312 5.207032 2.929688 7.066406 1.859374 1.863282 4.441406 2.929688 7.070312 2.929688s5.210938-1.066406 7.070312-2.929688c1.859376-1.859374 2.929688-4.4375 2.929688-7.066406 0-2.632812-1.070312-5.210937-2.929688-7.070312-1.859374-1.859375-4.441406-2.929688-7.070312-2.929688zm0 0"></path><path d="m99.457031 389.417969c2.558594 0 5.121094-.976563 7.070313-2.925781 3.90625-3.90625 3.90625-10.238282 0-14.144532l-6.925782-6.929687h59.101563c14.335937 0 26 11.664062 26 26 0 14.335937-11.664063 26-26 26h-35.019531c-5.523438 0-10 4.476562-10 10 0 5.519531 4.476562 9.996093 10 9.996093h35.019531c25.363281 0 46-20.632812 46-45.996093s-20.636719-45.996094-46-45.996094h-59.101563l6.925782-6.929687c3.90625-3.90625 3.90625-10.238282 0-14.144532-3.902344-3.902344-10.234375-3.902344-14.140625 0l-24 24c-3.902344 3.90625-3.902344 10.238282 0 14.144532l24 23.996093c1.953125 1.953125 4.511719 2.929688 7.070312 2.929688zm0 0"></path><path d="m46.074219 476.449219c-2.84375 0-5.667969-1.210938-7.640625-3.558594l-.019532-.023437c-3.554687-4.226563-3.007812-10.53125 1.21875-14.085938 4.226563-3.558594 10.535157-3.011719 14.089844 1.214844 3.550782 4.230468 3.015625 10.546875-1.210937 14.101562-1.878907 1.578125-4.164063 2.351563-6.4375 2.351563zm0 0"></path></svg>
                                                </div>
                                                <div class="Detaila">
                                                    <h2>30 Days Return</h2>
                                                    <p>Simply return it within 30 days for an exchange</p>
                                                </div>
                                            </div>
                                        </div>
                                        
                                        <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
                                            <div class="PropServApp_Item">
                                                <div class="icon">
                                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" width="512" height="512" src="assets/images/icons/service_4.svg" class="svg_img" alt=""><g id="_04-security" data-name="04-security"><g id="linear_color-2" data-name="linear color"><path d="M160,207H48a10,10,0,0,0-10,10v32a10,10,0,0,0,10,10H160a10,10,0,0,0,10-10V217A10,10,0,0,0,160,207Zm-10,32H58V227h92Z"></path><path d="M112,278H49a10,10,0,0,0,0,20h63a10,10,0,0,0,0-20Z"></path><path d="M506.347,328.917c-1.114-29.639-3.358-57.449-3.38-57.727a10,10,0,0,0-7.151-8.785L442,246.605V80a26.029,26.029,0,0,0-26-26H32A26.029,26.029,0,0,0,6,80V336a26.029,26.029,0,0,0,26,26H261.022c.157,28.336,2.652,37.709,5.034,42.472,1.785,3.57,6.526,13.053,60.4,57.92,25.953,21.615,51.141,41.294,51.393,41.49a10,10,0,0,0,12.306,0c.252-.2,25.44-19.875,51.393-41.49,53.872-44.867,58.613-54.35,60.4-57.92C505.08,398.2,508.414,383.945,506.347,328.917ZM26,138H422v28H26Zm6-64H416a6.006,6.006,0,0,1,6,6v38H26V80A6.006,6.006,0,0,1,32,74ZM26,336V186H422v54.734l-35.184-10.329a9.992,9.992,0,0,0-5.632,0l-109,32a10,10,0,0,0-7.151,8.785c-.022.278-2.266,28.088-3.38,57.727-.174,4.647-.309,9-.41,13.083H32A6.006,6.006,0,0,1,26,336Zm458.056,59.528C479.3,405.038,430.55,446.661,384,483.294c-46.549-36.631-95.3-78.253-100.056-87.766-.625-1.25-3.279-8.655-2.881-43.382,0-.049.007-.1.007-.146,0-.031,0-.061,0-.091.076-6.421.255-13.768.571-22.179.768-20.486,2.086-40.1,2.812-50.082L384,250.422l99.551,29.226c.725,9.969,2.041,29.544,2.81,50.019C488.4,383.89,484.8,394.043,484.056,395.528Z"></path><path d="M457.851,292.947l-71.034-20.854a10,10,0,0,0-5.634,0l-71.034,20.854a10,10,0,0,0-7.165,9.006c-1.709,28.945-2.9,65.572-1.106,81.565a9.992,9.992,0,0,0,2.621,5.7c12.194,13.09,45.82,41.377,73.222,63.484a10,10,0,0,0,12.558,0c27.4-22.107,61.028-50.394,73.222-63.484a9.992,9.992,0,0,0,2.621-5.7c1.79-15.993.6-52.621-1.106-81.565A10,10,0,0,0,457.851,292.947ZM446.54,378c-10.431,10.483-34,30.861-62.54,54.058-28.535-23.2-52.109-43.575-62.54-54.058-.908-12.826-.5-38.308,1.093-67.848L384,292.11l61.447,18.04C447.04,339.689,447.448,365.171,446.54,378Z"></path><path d="M422.247,320.191a10,10,0,0,0-14.056,1.562l-26.272,32.841L365.547,343.68a10,10,0,0,0-11.094,16.64l24,16a10,10,0,0,0,13.356-2.073l32-40A10,10,0,0,0,422.247,320.191Z"></path></g></g></svg>
                                                </div>
                                                <div class="Detaila">
                                                    <h2>Payment Secure</h2>
                                                    <p>Contact us 24 hours a day, 7 days a week</p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                
                               </div>
                        
                      </div>
                    </div>

                    <div class="footbot">
                      <div class="styacont">
                        <h2>Stay Connected</h2>
                        <div class="social">

                          <span>
                            <a href="#">
                              <i class="fab fa-facebook-f"> </i>
                            </a>
                          </span>
                          <span>
                            <a href="#">
                              <i class="fab fa-linkedin-in"> </i>
                            </a>
                          </span>
                          <span>
                            <a href="#">
                              <i class="fab fa-instagram"> </i>
                            </a>
                          </span>

                          <span>
                            <a href="#">
                              <i class="fab fa-twitter"> </i>
                            </a>
                          </span>

                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">

                </div>
              </div>
            </div>
          </div>
          
          
          
          

@endsection